...
class Numbers_Words {
	 ...
	 *        	
	 * @param string $locale
	 *        	Language name abbreviation. Optional. Defaults to en_US.
	 *        	
	 ...
	 * @since PHP 4.2.3
	 */
	public static function toWords($num, $locale) {
		if ($locale == '') {
			$locale = 'en_US';
		}
	 ...
	 *        	International currency symbol
	 * @param
	 *        	mixed string/array of strings $locale
		...
		if ($dh) {
			while ( $fname = readdir ( $dh ) ) {
				if (preg_match ( '#^lang\.([a-z_]+)\.php$#i', $fname, $matches )) {
					if (is_file ( $dname . $fname ) && is_readable ( $dname . $fname ) && (! isset ( $locale ) || in_array ( $matches [1], $locale ))) {
						$ret [] = $matches [1];
					}
				}
			}

...
if (isset($_POST ['CommitBatch']) and ($_POST ['CommitBatch'] == _('Aceptar y Procesar Nueva Póliza...') or $_POST ['CommitBatch'] == _('Actualizar Modificaciones...'))) {
    ...
    if ($PeriodNo != -999) {
        ...
        // Actualizar folio ue
        $SQL = "UPDATE gltrans SET nu_folio_ue = 0 WHERE typeno='" . $TransNo . "' and type='" . $TipoPoliza . "'";
        $result = DB_query($SQL, $db);
        ...
        $url = "&FromCust=>1&ToCust=>1&PrintPDF=>Yes&type=>$TipoPoliza&TransNo=>$TransNo&periodo=>$p&trandate=>$datejournal&folioUe=>0&ue=>$JournalItem->ue";
        $enc = new Encryption;
        $url = $enc->encode($url);
        $liga= "URL=" . $url;
        // echo "<br><a href='PrintJournal.php?$liga' target='_blank'><b><img src='images/printer.png' title='Póliza' alt=''>&nbsp;&nbsp; Imprime Póliza</b></a>";

        echo '<br><a href="PrintJournal.php?'.$liga.'" target="_blank"><span class="glyphicon glyphicon-print" aria-hidden="true"></span>' . _ ( ' Imprime Póliza ' ) . '</a>';
        ...
        echo '<a id="linkRegresar" name="linkRegresar" href="ReporteGLJournal.php" class="btn btn-default botonVerde glyphicon glyphicon-share-alt" title="Regresar"> Regresar</a> ';

        echo '<a id="linkNuevo" name="linkNuevo" href="GLJournal.php?'.$liga.'" class="btn btn-default botonVerde glyphicon glyphicon-plus" title="Nuevo"> Nuevo</a> ';
    ...
    if ($_POST['cmbTipoPoliza'] == '282') {
        ...
        $_POST['textoVisible__selectGLNarrative'] = $_SESSION ['JournalDetail']->extraPresupuestalTex;

        // Eliminar partidas seleccionadas ya que modifico
        unset($_SESSION ['JournalDetail']->GLEntries);
        unset($_SESSION ['JournalDetail']->extraPresupuestalTex);
    ...
    if ($_POST['cmbTipoPoliza'] == '282') {
        ...
        $_SESSION ['JournalDetail']->extraPresupuestalTex = $_POST['textoVisible__selectGLNarrative'];
        $SQL = "SELECT tb_matriz_extraptal.categorydescription  FROM tb_matriz_extraptal WHERE tb_matriz_extraptal.id = '".$_POST['selectGLNarrative']."'";
        $TransResult = DB_query($SQL, $db);
    ...
    if (!empty($_POST ['txtPresupuestalEgresosCargo']) and is_numeric($montocargo)) {
        ...
        if ($AllowThisPosting) {
            ...
            $SQL = "SELECT accountcode
                ...
                WHERE accountcode= '".$_POST ['txtPresupuestalEgresosCargo']."'";
            
            $Result = DB_query($SQL, $db);/*cga*/ //echo '<br>24:'.$SQL;

            if ($registro= DB_fetch_array($Result)) {
                $nombrecargo= $registro["accountcode"];
            }
            ...
            if (DB_num_rows($Result) == 0) {
                ...
                $_SESSION ['JournalDetail']->Add_To_GLAnalysis(
                    $montocargo,
                    $_POST ['GLNarrative'],
                    $_POST ['txtPresupuestalEgresosCargo'],
                    $nombrecargo,
                    $_POST ['tag'],
    ...
    if (!empty($_POST ['txtPresupuestalIngresosCargo']) and is_numeric($montocargo)) {
        // If a manual code was entered need to check it exists and isnt a bank account
        $AllowThisPosting = true; // by default
                                  // SE CREO LA FUNCION PARA DAR PERMISOS A CIERTOS USUARIOS MODIFICAR CUENTSA DE CLIENTES Y PROVEEDORES
                                  // echo "<br>entra aki";
        ...
        if ($AllowThisPosting) {
            ...
            $SQL = "SELECT accountcode
                FROM chartdetailsbudgetbytag
                WHERE accountcode= '".$_POST ['txtPresupuestalIngresosCargo']."'";
            ...
            if (DB_num_rows($Result) == 0) {
                ...
                $_SESSION ['JournalDetail']->Add_To_GLAnalysis(
                    $montocargo,
                    $_POST ['GLNarrative'],
                    $_POST ['txtPresupuestalIngresosCargo'],
                    $nombrecargo,
                    $_POST ['tag'],
                    $_SESSION ['JournalDetail']->JnlLegalId,
                    $_POST ['rate'],
    ...
    if (!empty($_POST ['txtPresupuestalEgresosAbono']) and is_numeric($montoabono)) {
        ...
        if ($AllowThisPosting) {
            ...
            $SQL = "SELECT accountcode
                FROM chartdetailsbudgetbytag
                WHERE accountcode= '".$_POST ['txtPresupuestalEgresosAbono']."'";
            
            $Result = DB_query($SQL, $db);/*cga*/ //echo '<br>24:'.$SQL;

            if ($registro= DB_fetch_array($Result)) {
                $nombreabono= $registro["accountcode"];
            }

            $_SESSION ['JournalDetail']->Add_To_GLAnalysis(
                $montoabono,
                $_POST ['GLNarrative'],
                $_POST ['txtPresupuestalEgresosAbono'],
                $nombreabono,
                $_POST ['tag'],
                $_SESSION ['JournalDetail']->JnlLegalId,
                $_POST ['rate'],
    ...
    if (!empty($_POST ['txtPresupuestalIngresosAbono']) and is_numeric($montoabono)) {
        $AllowThisPosting = true;
        ...
        if ($AllowThisPosting) {
            ...
            $SQL = "SELECT accountcode
                FROM chartdetailsbudgetbytag
                WHERE accountcode= '".$_POST ['txtPresupuestalIngresosAbono']."'";
            ...
            $_SESSION ['JournalDetail']->Add_To_GLAnalysis(
                $montoabono,
                $_POST ['GLNarrative'],
                $_POST ['txtPresupuestalIngresosAbono'],
                $nombreabono,
                $_POST ['tag'],
                $_SESSION ['JournalDetail']->JnlLegalId,
                $_POST ['rate'],
        ...
        <div id="idPanelPoliza" class="row panel-collapse collapse in ptb5" role="tabpanel" aria-labelledby="headingOne">
            ...
            <div class="row p5 m0">
                ...
                <div class="col-md-4" id="ColumnaDerecha">
                    <div class="form-inline row">
                        <div class="col-md-3">
                            <span><label>Programa <?= ( $_POST['cmbTipoPoliza']=='282' ? "Extra" : "Presu" ); ?>p: </label></span>
                        </div>
                        <div class="col-md-9">
                            <select name="cmbProgramaPresupuestario" id="cmbProgramaPresupuestario" class="form-control select selectProgramaPresupuestarioLocal" onchange="fnCambioDatosIdentificador()">
                                <option value="-1">Seleccionar...</option>
                                <?php
                                $SQL = ( $_POST['cmbTipoPoliza']=='282' ? "SELECT `pe` AS `cppt`, CONCAT(`pe`, ' - ', `descripcion`) AS `descripcion` FROM `tb_cat_programa_extrapresupuestario` WHERE `activo` = '1'" : "SELECT `cppt`, CONCAT(`cppt`, ' - ', `descripcion`) AS `descripcion` FROM `tb_cat_programa_presupuestario` ORDER BY `descripcion` ASC" );
                                $result = DB_query($SQL, $db);
                                while ($myrow = DB_fetch_array($result)) {
                                    $selected = ( $_POST['cmbProgramaPresupuestario']==$myrow['cppt'] ? "selected" : "" );
            ...
            <div class="col-md-2">
                        <div class="col-md-3">
                            <span><label>Tipo Cargo</label></span>
                        </div>
                        <div class="col-md-9">
                            <select name="tipoCargo" id="tipoCargo" class="form-control selectGeneral">
                                <option value='vacio'>Seleccionar...</option>
                                <option value='preEgresosCargo'>Presupuesto Egresos</option>
                                <option value='preIngresoCargo'>Presupuesto Ingreso</option>
                                <option value='contableCargo'>Contable</option>
                            </select>
                        </div>
            </div>

            <div id="divPreEgresosCargo" class="col-md-4" style="display: none;">
            <div>
                        <!-- <div class="col-md-3">
                        ...
                        <component-text-label label="Presupuesto Egreso Cargo: " id="txtPresupuestalEgresosCargo" name="txtPresupuestalEgresosCargo" placeholder="Buscar Presupuesto" title="Buscar Presupuesto" onkeypress="return fnObtenerPresupuestoBusqueda(event)" ></component-text> 
                    </div>
            </div>
            
            <div id="divPreIngresosCargo" class="col-md-4" style="display: none;">
            <div>
                        <!-- <div class="col-md-3">
                            <span><label>Cuenta Cargo: </label></span>
                        </div>
                        ...
                        <component-text-label label="Presupuesto Ingreso Cargo: " id="txtPresupuestalIngresosCargo" name="txtPresupuestalIngresosCargo" placeholder="Buscar Presupuesto" title="Buscar Presupuesto" onkeypress="return fnObtenerPresupuestoBusquedaIngreso(event)"></component-text> 
                    </div>
            </div>

            <div id="divContableCargo" class="col-md-4" style="display: none;">
            <div id="divCuentaCargoDiario" name="divCuentaCargoDiario" <?php echo $ocultaPolizaDiario; ?> >
                        <!-- <div class="col-md-3">
                            <span><label>Cuenta Cargo: </label></span>
                        </div>
                        <div class="col-md-9">
                        ...
                        <component-text-label label="Cuenta Cargo: " id="GLCode" name="GLCode" placeholder="Cuenta Cargo" title="Cuenta Cargo" onkeypress="fnValidarCargarCuentas(this)" value="<?php echo $cuentacargo; ?>"></component-text-label>
                    </div>
            </div>
                        ...
                        <div class="form-inline row">
                            ...
                            <div class="col-md-9">
                                <select name="cmbTipoPoliza" id="cmbTipoPoliza" class="form-control cmbTipoPoliza" onchange="fnCambioTipoPoliza(this)">
                                    ...
                                    <option value="120" <?php echo ($_POST['cmbTipoPoliza'] == '120' ? 'selected' : ''); ?>>Cierre de Caja</option>
                                </select>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-md-2">
                        <div class="col-md-3">
                            <span><label>Tipo Abono</label></span>
                        </div>
                        <div class="col-md-9">
                            <select name="tipoAbono" id="tipoAbono" class="form-control selectGeneral">
                                <option value='vacio'>Seleccionar...</option>
                                <option value='preEgresosAbono'>Presupuesto Egresos</option>
                                <option value='preIngresoAbono'>Presupuesto Ingreso</option>
                                <option value='contableAbono'>Contable</option>
                            </select>
                        </div>
            </div>

            <div id="divPreEgresosAbono" class="col-md-4" style="display: none;">
            <div>
                        <!-- <div class="col-md-3">
                            <span><label>Cuenta Cargo: </label></span>
                        </div>
                        ...
                        <component-text-label label="Presupuesto Egreso Abono: " id="txtPresupuestalEgresosAbono" name="txtPresupuestalEgresosAbono" placeholder="Buscar Presupuesto" title="Buscar Presupuesto" onkeypress="return fnObtenerPresupuestoBusquedaEgresoAbono(event)" ></component-text> 
                    </div>
            </div>
            
            <div id="divPreIngresosAbono" class="col-md-4" style="display: none;">
            <div>
                        <!-- <div class="col-md-3">
                            <span><label>Cuenta Cargo: </label></span>
                        </div>
                        ...
                        <component-text-label label="Presupuesto Ingreso Abono: " id="txtPresupuestalIngresosAbono" name="txtPresupuestalIngresosAbono" placeholder="Buscar Presupuesto" title="Buscar Presupuesto" onkeypress="return fnObtenerPresupuestoBusquedaIngresoAbono(event)"></component-text> 
                    </div>
            </div>

            <div id="divContableAbono" class="col-md-4" style="display: none;">
            <div id="divCuentaAbonoDiario" name="divCuentaAbonoDiario" <?php echo $ocultaPolizaDiario; ?>>
                        <!-- <div class="col-md-3">
                        ...
                        <component-text-label label="Cuenta Abono: " id="GLCodeAbono" name="GLCodeAbono" placeholder="Cuenta Abono" title="Cuenta Abono" onkeypress="fnValidarCargarCuentas(this)" value="<?php echo $cuentaabono; ?>"></component-text-label>
                    </div>
            </div>
            ...
            <div class="row p5 m0">
                ...
                <div class="col-md-12" id="divDescripcionExtraPre" name="divDescripcionExtraPre" <?php echo $ocultaPolizaExtraPre; ?>>
                    <div class="form-inline row">
                        ...
                        <div class="col-md-9">
                            <input type="text"  class="form-control campoListado"  placeholder="Descripción" class="form-control" style="width:100%" name="textoVisible__selectGLNarrative" id="textoVisible__selectGLNarrative" autocomplete="off" readonly value="<?= $_POST['textoVisible__selectGLNarrative']; ?>" />
                            <input type="hidden" name="selectGLNarrative" id="selectGLNarrative" value="<?= $_POST['selectGLNarrative']; ?>">
                            <input type="hidden" name="textoOculto__selectGLNarrative" id="textoOculto__selectGLNarrative" value="">
                            <div id ="sugerencia-selectGLNarrative" style="position:absolute; z-index:999; display:block;"></div><?php /*
                            ...
                            onchange="fnObtenerDatosExtraPres(this)">
                                <option selected value='0'>Sin selección</option>
                                <?php
                                //IF(`mep`.`ordendesplegar` IS NOT NULL&&`mep`.`ordendesplegar`<>'',CONCAT(' - ',`mep`.`ordendesplegar`), ''),
                                //IF(`tb_matriz_extraptal_proceso`.`txt_descripcion` IS NOT NULL, CONCAT(' - ',`tb_matriz_extraptal_proceso`.`txt_descripcion`), '')
                                $SQL = "SELECT `mep`.`id`, `mep`.`categoryid`, CONCAT(`mep`.`categorydescription`,
                                ...
                                $result = DB_query($SQL, $db);
                                while ($myrow = DB_fetch_array($result)) {
                                    $selected = ( $_POST['selectGLNarrative']==$myrow["id"] ? " selected" : "" );
                                    echo "<option$selected value=\"$myrow[id]\">$myrow[descripcion]</option>";
                                }
                                ?>
                            </select>*/ ?>
                        </div>
...
var tagrefBus = $('#tag').val();
var ueBus = $('#selUE').val();

if(tagrefBus == '-1' && ueBus == '-1'){   
     $("#GLCode").prop("readonly", true);
     $("#txtPresupuestalEgresosCargo").prop("readonly", true);
     $("#txtPresupuestalIngresosCargo").prop("readonly", true);
   
}

if(tagrefBus != '-1' && ueBus != '-1'){   
     $("#GLCode").prop("readonly", false);
     $("#txtPresupuestalEgresosCargo").prop("readonly", false);
     $("#txtPresupuestalIngresosCargo").prop("readonly", false);
 ...
 $('#GLCode').val("");
 $('#txtPresupuestalIngresosCargo').val("");	
 }
 ...
 document.getElementById('divContableCargo').style.display = 'none';
 $('#GLCode').val("");
 $('#txtPresupuestalEgresosCargo').val("");
 }
...
function fnObtenerPresupuestoBusqueda(panel="") {
	...
	dataObj = { 
	        option: 'obtenerPresupuestosBusquedaEgresos',
            tagref: tagrefBus,
            tipo: tipo,
	        ue: ueBus
	      };
...
function fnBusquedaReduccion(jsonData) {
	$( "#txtPresupuestalEgresosCargo").autocomplete({
        source: jsonData,
        select: function( event, ui ) {
            
            $( this ).val( ui.item.accountcode + "");
            $( "#txtPresupuestalEgresosCargo" ).val( ui.item.accountcode );
            // $( "#txtBuscarReducciones" ).val( "" );
 ...
 * @return {[type]}       [description]
 */
function fnObtenerPresupuestoBusquedaIngreso(panel="") {
	// console.log("fnObtenerPresupuestoBusqueda iskls");
	// Obtener claves de busqueda
	// if (type != '294') {
	// 	// Si no es directo no obtener claves
    ...
    .done(function( data ) {
        ...
        if(data.result) {
            //console.log("datosPresupuestosBusqueda: "+JSON.stringify(datosPresupuestosBusqueda));
            datosPresupuestosBusqueda = data.contenido.datos;
            fnBusquedaReduccionIngresos(datosPresupuestosBusqueda);
        }
    })
    .fail(function(result) {
        console.log( result );
 ...
 * @return {[type]}          [description]
 */
function fnBusquedaReduccionIngresos(jsonData) {
	$( "#txtPresupuestalIngresosCargo").autocomplete({
        source: jsonData,
        select: function( event, ui ) {
            
            $( this ).val( ui.item.accountcode + "");
            $( "#txtPresupuestalIngresosCargo" ).val( ui.item.accountcode );
            // $( "#txtBuscarReducciones" ).val( "" );
...
var ueBus = $('#selUE').val();
if(tagrefBus == '-1' && ueBus == '-1'){   
     $("#GLCodeAbono").prop("readonly", true);
     $("#txtPresupuestalEgresosAbono").prop("readonly", true);
     $("#txtPresupuestalIngresosAbono").prop("readonly", true);
   
}

if(tagrefBus != '-1' && ueBus != '-1'){   
     $("#GLCodeAbono").prop("readonly", false);
     $("#txtPresupuestalEgresosAbono").prop("readonly", false);
     $("#txtPresupuestalIngresosAbono").prop("readonly", false);
   
}

 if(tipoAbono == 'preEgresosAbono'){
 // 	// $('#fechaFinal').hide(); //oculto mediante id
 // 	// $('#fechaInicio').hide();
 document.getElementById('divPreEgresosAbono').style.display = 'block';
 document.getElementById('divPreIngresosAbono').style.display = 'none';
 document.getElementById('divContableAbono').style.display = 'none';
 $('#GLCodeAbono').val("");
 $('#txtPresupuestalIngresosAbono').val("");	
 }
         
 if(tipoAbono == 'preIngresoAbono'){
 
 document.getElementById('divPreEgresosAbono').style.display = 'none';
 document.getElementById('divPreIngresosAbono').style.display = 'block';
 document.getElementById('divContableAbono').style.display = 'none';
 $('#GLCodeAbono').val("");
 $('#txtPresupuestalEgresosAbono').val("");
 }
 
 if(tipoAbono == 'contableAbono'){
     
 document.getElementById('divPreEgresosAbono').style.display = 'none';
 document.getElementById('divPreIngresosAbono').style.display = 'none';
 document.getElementById('divContableAbono').style.display = 'block';
 $('#txtPresupuestalIngresosAbono').val("");
 $('#txtPresupuestalEgresosAbono').val("");	
 }
 ...
 * @return {[type]}       [description]
 */
function fnObtenerPresupuestoBusquedaIngresoAbono(panel="") {
	// console.log("fnObtenerPresupuestoBusqueda iskls");
	// Obtener claves de busqueda
	// if (type != '294') {
	// 	// Si no es directo no obtener claves
    ...
    .done(function( data ) {
        ...
        if(data.result) {
            //console.log("datosPresupuestosBusqueda: "+JSON.stringify(datosPresupuestosBusqueda));
            datosPresupuestosBusqueda = data.contenido.datos;
            fnBusquedaReduccionIngresosAbono(datosPresupuestosBusqueda);
        }
    })
    .fail(function(result) {
        console.log( result );
 ...
 * @return {[type]}          [description]
 */
function fnBusquedaReduccionIngresosAbono(jsonData) {
	$( "#txtPresupuestalIngresosAbono").autocomplete({
        source: jsonData,
        select: function( event, ui ) {
            
            $( this ).val( ui.item.accountcode + "");
            $( "#txtPresupuestalIngresosAbono" ).val( ui.item.accountcode );
            // $( "#txtBuscarReducciones" ).val( "" );
 ...
 * @return {[type]}       [description]
 */
function fnObtenerPresupuestoBusquedaEgresoAbono(panel="") {
	// console.log("fnObtenerPresupuestoBusqueda iskls");
	// Obtener claves de busqueda
	// if (type != '294') {
	// 	// Si no es directo no obtener claves
    ...
    .done(function( data ) {
        ...
        if(data.result) {
            //console.log("datosPresupuestosBusqueda: "+JSON.stringify(datosPresupuestosBusqueda));
            datosPresupuestosBusqueda = data.contenido.datos;
            fnBusquedaReduccionEgresos(datosPresupuestosBusqueda);
        }
    })
    .fail(function(result) {
        console.log( result );
 ...
 * @return {[type]}          [description]
 */
function fnBusquedaReduccionEgresos(jsonData) {
	$( "#txtPresupuestalEgresosAbono").autocomplete({
        source: jsonData,
        select: function( event, ui ) {
            
            $( this ).val( ui.item.accountcode + "");
            $( "#txtPresupuestalEgresosAbono" ).val( ui.item.accountcode );
            // $( "#txtBuscarReducciones" ).val( "" );

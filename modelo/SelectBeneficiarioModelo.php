   ...
   $SQL = "SELECT count(*) as total FROM tb_bancos_proveedores WHERE ln_supplierid='".$idProvs."' AND ln_bank_id='".$bank."' AND nu_cuenta='".$cuenta."' OR nu_clabe_interbancaria='".$clabe."'";
    ...
    case 'mostrarProveedores': 
      ...
      $SQL = "SELECT suppliers.*
              FROM suppliers where id_nu_tipo = '3'";
    ...
    $contenido = array('datosProveedor' => $info, 'nombreExcel' => $nombreExcel);
    $result = true;
    ...
    case 'cuentasContables':
      ...
      $SQL = "SELECT `accountcode` AS valor, `accountname` AS label, `group_` as padre
              FROM chartmaster
              WHERE nu_nivel >= 5 AND `accountcode` LIKE '2.1.1%'

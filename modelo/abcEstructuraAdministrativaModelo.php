...
define('LNTBLCAT', 'tb_cat_estructura_administrativa');
define('ACTIVECOL', 'ind_activo');
define('DATA', [
	'ur'=>['col'=>'id_nu_ur','tipo'=>'string'],
	'aux'=>['col'=>'id_nu_auxiliar','tipo'=>'string'],
	'ef'=>['col'=>'id_nu_ef','tipo'=>'string'],
	'identificador'=>['col'=>'id_nu_estructura_administrativa','tipo'=>'string'],
]);
define('CONCAT', "CONCAT(`id_nu_ur`,'-',`id_nu_auxiliar`,'-',`id_nu_ef`)");
...
function store($db)
{
	...
	if(compruebaClaveAdministrativa($db, $clave) && !$valid){
...
function show($db)
{
	...
	$sql = "SELECT `id_nu_estructura_administrativa` as id,`id_nu_ur` as ur,`id_nu_auxiliar` as aux,`id_nu_ef` as ef
		,concat(`id_nu_ur`,'-',`id_nu_auxiliar`,'-',`id_nu_ef`) as descripcion
		FROM ".LNTBLCAT." WHERE `".ACTIVECOL."`='1' ORDER BY `id_nu_ur` ,`id_nu_auxiliar`,`id_nu_ef`";
	...
	while ($rs = DB_fetch_array($result)) {
		$data['content'][] = [
            ...
            'aux'=>$rs['aux'],// 1
            'ef'=>$rs['ef'],// 2
            'descripcion'=>$rs['descripcion'],// 3
            'identificador'=>$rs['id'],// 7
...
function update($db)
{
	...
	$registroInactivo = compruebaClaveAdministrativaInactivas($db, $clave);
	// comprobacion de existencia
	if(compruebaClaveAdministrativa($db, $clave) && !$registroInactivo){
		$data['msg'] = "La clave $clave ya se encuentra registrada.";
		$data['content'] = show($db)['content'];
		return $data;
	}
	...
	try {
		...
		if($resultUpdate==true){
			$data['msg'] = "Se actualizo la información con éxito";
			$data['success'] = true;
			DB_Txn_Commit($db);
			$data['content'] = show($db)['content'];
		}else{
...
{
	$sqlCampos = "select `campoPresupuesto` as dato, `nu_administrativa_orden` from budgetConfigClave where `nu_administrativa` = 1 ORDER BY `nu_administrativa_orden`";
	...
	# consulta de clave
	$sqlCalve = "select concat(`id_nu_ur`,'-',`id_nu_auxiliar`,'-',`id_nu_ef`) as clave from `tb_cat_estructura_administrativa` where `id_nu_estructura_administrativa` = '$identificador'";
...
function compruebaClaveAdministrativaInactivas($db, $clave)
{
	$sql = "SELECT CONCAT(`id_nu_ur`,'-',`id_nu_auxiliar`,'-',`id_nu_ef`)
		FROM `tb_cat_estructura_administrativa` WHERE `ind_activo`='0' AND CONCAT(`id_nu_ur`,'-',`id_nu_auxiliar`,'-',`id_nu_ef`) = '$clave'";
	$result = DB_query($sql, $db);

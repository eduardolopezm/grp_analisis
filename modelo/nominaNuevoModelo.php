...
if ($option == 'procesarNomina'){
    ...
    tb_file_nomina.ur, 
    tb_file_nomina.pp, 
    tb_file_nomina.partida, 
    SUM(tb_file_nomina.importe) AS importe,
    chartdetailsbudgetbytag.accountcode as clave,
    tb_cat_unidades_ejecutoras.ue
    FROM tb_file_nomina
    JOIN chartdetailsbudgetbytag ON chartdetailsbudgetbytag.anho = '$anioFiscal'
    AND chartdetailsbudgetbytag.cppt = tb_file_nomina.pp 
    AND partida_esp = tb_file_nomina.partida 
    AND chartdetailsbudgetbytag.ln_aux1 = CONCAT(tb_file_nomina.ur,tb_file_nomina.ue)
    ...
    GROUP BY tb_file_nomina.pp, tb_file_nomina.partida
    ORDER BY tb_file_nomina.pp, tb_file_nomina.partida";
    ...
    while ($myrow = DB_fetch_array($TransResult)) {
        ...
        $UR           =  $myrow ['ur'] ;
        $UE           =  $myrow ['ue'] ;
        $totalcompra  =  abs($myrow ['importe']); 
        $clave        =  $myrow["clave"];
        $partida      =  $myrow["partida"];
        $folioCon = GetNextTransNo(305, $db);
       
        //***** Comprometido
        $folioPolizaUe  = fnObtenerFolioUeGeneral($db, $UR, $UE, 305);
        ...
        // Log Presupuesto
        // $agregoLog = fnInsertPresupuestoLog($db, 305, $folioCon, $UR, $clave, $period, $totalcompra, 258, "", 'DISPONIBLE - COMPROMISO', 1, '', 0, $UE); // Abono
        $agregoLog = fnInsertPresupuestoLog($db, 305, $folioCon, $UR, $clave, $period, $totalcompra * -1, 259, "", 'DISPONIBLE - COMPROMISO', 1, '', 0, $UE); // Cargo
        ...
        // Log Presupuesto
        $agregoLog = fnInsertPresupuestoLog($db, 305, $folioCon, $UR, $clave, $period, $totalcompra, 259, "", 'COMPROMETIDO-DEVENGADO', 1, '', 0, $UE); // Abono
        $agregoLog = fnInsertPresupuestoLog($db,305, $folioCon, $UR, $clave, $period, $totalcompra * -1, 260, "",'COMPROMETIDO-DEVENGADO', 1, '', 0, $UE); // Cargo
        ...
        $agregoLog = fnInsertPresupuestoLog($db, 305, $folioCon, $UR, $clave, $period, $totalcompra, 260, "", 'EJERCIDO - DEVENGADO', 1, '', 0, $UE); // Abono
        $agregoLog = fnInsertPresupuestoLog($db, 305, $folioCon, $UR, $clave, $period, $totalcompra * -1, 261, "", 'EJERCIDO - DEVENGADO', 1, '', 0, $UE); // Cargo
        ...
        // Log Presupuesto
        $agregoLog = fnInsertPresupuestoLog($db, 305, $folioCon, $UR, $clave, $period, $totalcompra, 261, "", 'EJERCIDO - PAGADO', 1, '', 0, $UE); // Abono
        $agregoLog = fnInsertPresupuestoLog($db, 305, $folioCon, $UR, $clave, $period, $totalcompra * -1, 265, "", 'EJERCIDO - PAGADO', 1, '', 0, $UE); // Cargo

        // Reduce disponible del presupuesto
        // $folioCon  = GetNextTransNo(305, $db);
        $periodo    = GetPeriod(date('d/m/Y',strtotime($fechaInicio)), $db);
        $qty       = $totalcompra * -1; 
        // $respuesta = fnInsertPresupuestoLogAcomulado($db, 305, $folioCon, $UR, $clave, $period,$qty ,265, "", "PAGO DE NOMINA DE LA CLAVE $clave", 1, '', 0, '09', 'DESC', 'disponible', '', 'Reduccion', 1,'', '', 0);
    ...
    $TransResult                = DB_query($sql, $db, $ErrMsg);
    $idProcesoNomina            = fnIdNomina($db); 
    $periodo    = GetPeriod(date('d/m/Y',strtotime($fechaInicio)), $db);
    while ($myrow  = DB_fetch_array($TransResult)) {
        $ue               =  $myrow ['ue'] ;  
        $folioPolizaUe    = fnObtenerFolioUeGeneral($db, $UR, $ue, 305);
        $generarPoliza    = fnGeneraPoliza($ue,$UR,$db,$periodo,$fechaCaptura,$folioPolizaUe,$noQuincena,$idProcesoNomina); 
    }
...
if ($option == 'infoArchivo'){
    ...
    {   
        ...
        while (($datos  = fgetcsv($archivo, ",")) == true) {
            ...
            // Validar si importe viene vacio 
...
function fnGeneraPoliza ($ue,$UR,$db,$periodo,$fechaCaptura,$folioPolizaUe,$noQuincena,$idProcesoNomina){
    $cuentaContable = null; 
    $sql       =    "SELECT  ue, fn.desc_concepto,importe,cta_contable,fn.tipo_concepto
    FROM tb_file_nomina fn
    INNER  JOIN tb_cat_concepto_nomina  cn ON ( cve_concepto <> '0' AND cve_concepto = clave_concepto  AND cn.pp =  fn.pp  AND cn.partida =  fn.partida AND cn.tipo_concepto  =  fn.tipo_concepto  ) OR
    ( cve_concepto = '0' AND cve_concepto = clave_concepto  AND cn.pp =  fn.pp  AND cn.partida =  fn.partida AND  fn.tipo_concepto = '' AND cn.tipo_concepto  =  'D' )
    WHERE  activo = 1 AND importe > 0
     AND fn.pp = cn.PP AND fn.partida = cn.partida AND ue = '$ue'";
    $ErrMsg   = ''; 
    $sqlRespuesta = DB_query($sql, $db, $ErrMsg);
...
function fnIdNomina ( $db){
    $idProcesoNomina= null; 
    $sqlIdConcepto  = "   SELECT id_proceso_nomina   FROM tb_proceso_nomina   ORDER BY id_proceso_nomina DESC LIMIT 1";
    $ErrMsg         = ''; 
    $sqlExec    = DB_query($sqlIdConcepto, $db, $ErrMsg);
    while ($myrow = DB_fetch_array($sqlExec)) {
        $idProcesoNomina        = $myrow["id_proceso_nomina"]; 
    }
    return $idProcesoNomina;
}
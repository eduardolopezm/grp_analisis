...
if($option == 'encryptarURL'){
    ...
    $selectObjetoParcial = $_POST['selectObjetoParcial'];
    $aleatorioPrincipal = $_POST['aleatorioPrincipal'];
    $aleatorioParcial = $_POST['aleatorioParcial'];

    $_SESSION['selectObjetoPrincipal'.$aleatorioPrincipal] = '';
    if (!empty($_POST['selectObjetoPrincipal'])) {
        $_SESSION['selectObjetoPrincipal'.$aleatorioPrincipal] = $_POST['selectObjetoPrincipal'];
    }

    $_SESSION['selectObjetoParcial'.$aleatorioParcial] = '';
    if (!empty($_POST['selectObjetoParcial'])) {
        $_SESSION['selectObjetoParcial'.$aleatorioParcial] = $_POST['selectObjetoParcial'];
    }

    $url = $enc->encode($url);
    $liga= "URL=". $url;
    $Mensaje='PrintSituacionIngresos.php?'.$liga;
    $result = true;
}

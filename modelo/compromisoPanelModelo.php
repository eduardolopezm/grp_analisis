...
if ($option == 'obtenerSificiencia') {
    ...
    if ($ue != '') {
        $sqlWhere .= " AND (tb_compromiso.ln_ue IN (".$ue.") OR chartdetailsbudgetlog.ln_ue IN (".$ue.")) ";
    }
    ...
    JOIN sec_unegsxuser ON sec_unegsxuser.tagref = tb_compromiso.sn_tagref AND sec_unegsxuser.userid = '".$_SESSION['UserID']."'
    JOIN `tb_sec_users_ue` ON `tb_sec_users_ue`.`userid` = '".$_SESSION['UserID']."' AND tb_compromiso.sn_tagref = `tb_sec_users_ue`.`tagref` AND  tb_compromiso.ln_ue = `tb_sec_users_ue`.`ue`

    WHERE tb_compromiso.nu_type = 259 AND tb_compromiso.nu_anio_fiscal = '".$_SESSION['ejercicioFiscal']."' ".$sqlWhere."
    ...
    while ($myrow = DB_fetch_array($TransResult)) {
        ...
        $liga= "URL=" . $url;
        $operacion = '<a type="button" id="btnAbrirAutorizarAdecuacion_'.$myrow['nu_transno'].'" name="btnAbrirAutorizarAdecuacion_'.$myrow['nu_transno'].'" href="compromiso.php?'.$liga.'" title="Detalle Compromiso" style="color: blue;">'.$myrow ['nu_transno'].'</a>'; // target="_blank"
        
        $impresion = '<a type="button" id="btnImprimir'.$myrow['nu_transno'].'" name="btnImprimir'.$myrow['nu_transno'].'" href="impresionCompromiso.php?'.$liga.'" title="Imprimir Detalle" target="_blank"><span class="glyphicon glyphicon glyphicon-print"></span></a>';

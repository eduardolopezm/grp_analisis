...
define('LNTBLCAT', 'tb_cat_objetos_contrato');
define('IDENTIFICADOR', 'id_objetos');
define('ACTIVECOL', 'ind_activo');
define('DATA', [
		'selectObjetoPrincipal' => ['col' => 'id_loccode', 'tipo' => 'string'],
		'selectObjetoParcial' => ['col' => 'id_stock', 'tipo' => 'string'],
		'valor' => ['col' => 'amt_valor', 'tipo' => 'string'],
		'cantidad' => ['col' => 'nu_cantidad', 'tipo' => 'string'],
		'metros' => ['col' => 'ln_metros', 'tipo' => 'string'],
		'anio' => ['col' => 'nu_anio', 'tipo' => 'string'],
		'selectEstatus' => ['col' => 'ind_activo', 'tipo' => 'string'],
		'contrato' => ['col' => 'id_contratos', 'tipo' => 'string'],
		'identificador'=>['col'=>'id_objetos','tipo'=>'string']
]);
...
function show($db){
	...
	$sql = "SELECT
	objetosContrato.id_objetos as clave,
    ...
    objetosContrato.amt_valor as valor, 
    objetosContrato.nu_cantidad as cantidad, 
    objetosContrato.ln_metros as metros
	FROM ".LNTBLCAT." AS objetosContrato
    JOIN stockmaster on (stockmaster.stockid = objetosContrato.id_stock)
    JOIN locstock ON (locstock.stockid = objetosContrato.id_stock)
	JOIN locations on (locations.loccode = objetosContrato.id_loccode)
	WHERE objetosContrato.id_contratos = '".$_POST['contrato']."'
	AND objetosContrato.ind_activo = '1'";
	...
	while ($rs = DB_fetch_array($result)) {
		$data['content'][] = [
			...
			'metros'=>utf8_encode($rs['metros']),// 3

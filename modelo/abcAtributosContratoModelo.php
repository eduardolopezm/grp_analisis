...
define('LNTBLCAT', 'tb_cat_atributos_contrato');
define('IDENTIFICADOR', 'id_atributos');
define('ACTIVECOL', 'ind_activo');
define('DATA', [
		'contrato' => ['col' => 'id_contratos', 'tipo' => 'string'],
		'etiqueta' => ['col' => 'ln_etiqueta', 'tipo' => 'string'],
		'identificador'=>['col'=>'id_atributos','tipo'=>'string']
]);
...
function show($db){
	...
	$sql = "SELECT
	atributosContrato.id_atributos as clave,
    atributosContrato.id_contratos as contrato, 
    atributosContrato.ln_etiqueta as etiqueta
	FROM ".LNTBLCAT." AS atributosContrato
	WHERE atributosContrato.id_contratos = '".$_POST['contrato']."'
	AND atributosContrato.ind_activo = '1'";
	...
	while ($rs = DB_fetch_array($result)) {
		$data['content'][] = [
			'clave'=>utf8_encode($rs['clave']),// 1
			'contrato'=>utf8_encode($rs['contrato']),// 2
			'etiqueta'=>utf8_encode($rs['etiqueta']),// 3
...
function destroy($db){
	...
	if(empty($info['identificador'])){
		$data['msg'] = 'Es necesario indicar el identificador del atributo';
		return $data;
	}

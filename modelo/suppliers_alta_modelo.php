      ...
      case 'multiplesConceptos':
       ...
       /* $SQL="SELECT pargcalculado AS value,descripcion as texto FROM tb_cat_partidaspresupuestales_partidagenerica WHERE SUBSTRING(pargcalculado,1,2) IN (".$conceptosSql.")" ; */
        ...
        $moneda = $_POST['moneda'];
                ...
                if($param_name!='partidapresupuestal' && ($param_name!='presuuestaloncepto')   && ($param_name!='partidagenerica')  ){
                  ...
                  if($param_name=='partidagenerica'){
                   ...
                   for($d=0;$d<(count($partidas));$d++){
                    ...
                    $sql = "INSERT INTO suppliers (supplierid,suppname,address1,address2,address3,address4,address5,address6,suppliersince,bankpartics,bankref,bankact,taxid,email,tipodetercero,typeid,currcode,paymentterms,ln_tipoPersona,ln_curp,ln_representante_legal,nu_interior,nu_exterior,id_nu_entidad_federativa,id_nu_municipio,active,nu_tesofe,id_nu_tipo)
                    ...
                    $ErrMsg = "Error al guardar proveedor";
                    $DbgMsg = _('Fallo al al de alta partida con proveedor');
                    $result = DB_query($sql, $db, $ErrMsg, $DbgMsg);
            ...
            // Retenciones Inicio
            $_POST['retencionesProveedor'] = ( $_POST['retencionesProveedor']!="null" ? $_POST['retencionesProveedor'] : "" );
            $retencionesProveedor = $_POST['retencionesProveedor'];
            $retencionesProveedor = explode(",", $retencionesProveedor);

            $SQL = "DELETE FROM tb_suppliersRetencion WHERE supplierid = '".$data['SupplierID']."'";
            $ErrMsg='Error al eliminar retenciones del proveedor';
            $TransResult = DB_query($SQL, $db, $ErrMsg);

            if($_POST['retencionesProveedor']){

                foreach ($retencionesProveedor as $dato) {
                    // Agregar Retenciones
                    if (!empty($dato)) {
                        $SQL = "INSERT INTO tb_suppliersRetencion 
                        (supplierid, idret)
                        VALUES
                        ('".$data['SupplierID']."', '".$dato."')";
                        $ErrMsg='Error al agregar la retención del proveedor';
                        $TransResult = DB_query($SQL, $db, $ErrMsg);
                    }
                }
            }
        ...
        case 'modificarInfo':
            ...
            $sql = "UPDATE suppliers  SET
                    ...
                    "currcode='".$moneda."',".
                    "paymentterms='"."01"."',
                    nu_tesofe='".$_POST['checkTesofe']."'
                    WHERE supplierid='".$data['SupplierID']."'";

            $ErrMsg = "Error al modificar proveedor";
            $DbgMsg = _('Fallo al al de alta partida con proveedor');
            $result = DB_query($sql, $db, $ErrMsg, $DbgMsg);
...
devengando';
    ...
    break;
        ...
        $partidas=$_POST['partidas'];
            ...
            $ErrMsg = "Error al consultar partidas";
            $TransResult = DB_query($sql, $db, $ErrMsg);
            $i=0;

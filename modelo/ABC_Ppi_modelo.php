...
case 'mostrarCatalogo':
    $pyin=$_POST['identificador'];
    
    $sqlppi=  " WHERE activo = 'S' ";
    if (!empty($pyin) && ($pyin!='0')) {
        $sqlppi = " WHERE `id_nu_pp` = '".trim($_POST['identificador'])."' ";
    }

    $info = array();

    $SQL = "SELECT `id_nu_pp`,pyin,nomb,descripcion,ramo,cunr,inicio,fin,total,inv_ejercida,fact FROM g_cat_ppi ".$sqlppi."   ORDER BY pyin asc";
    ...
    while ( $myrow = DB_fetch_array($TransResult) ) {
        if (!empty($_POST['identificador'])) {
            ...
            'Modificar' => '<a onclick="fnModificar(\''.$myrow ['id_nu_pp'].'\')"><span class="glyphicon glyphicon-edit"></span></a>', 
            'Eliminar' => '<a onclick="fnEliminar(\''.$myrow ['id_nu_pp'].'\',\''.$myrow ['pyin'].'\')"><span class="glyphicon glyphicon-trash"></span></a>' );
        }else{
    ...
    $columnasNombresGrid .= " { text: 'Ramo', datafield: 'ramo', width: '5%', hidden: false },";
...
case 'AgregarCatalogo':
    ...
    if ($proceso == 'Modificar') {
        $SQL = "UPDATE g_cat_ppi SET descripcion = '$descripcion', nomb= '$nombre', ramo='$ramo', cunr='$cunr',total='$total',inv_ejercida='$inv_ejercida', inicio=STR_TO_DATE('$fecha','%d-%m-%Y %H:%i:%s'),fin= STR_TO_DATE('$fechaFinal','%d-%m-%Y %H:%i:%s'),
	    fact= STR_TO_DATE('$fact','%d-%m-%Y %H:%i:%s')
         WHERE `id_nu_pp` = '".$identificador."'";
        $ErrMsg = "No se modificó el registro para <b>".$pyin."</b> del Catálogo Programa Proyecto de Inversión.";
        $TransResult = DB_query($SQL, $db, $ErrMsg);
        $contenido = "Se modificó el registro <b>".$pyin."</b> del Catálogo Programa Proyecto de Inversión con éxito." ;
        $result = true;
    }
    else
    {
        ...
        if (DB_num_rows($TransResult) == 0) {
            ...
            $SQL = "INSERT INTO g_cat_ppi (`pyin`, `descripcion`, `fecha`)
		            ...
		            */
                    ...
                    $SQL = "INSERT INTO g_cat_ppi (`pyin`, `descripcion`, `inicio`,`nomb`,`ramo`,`cunr`,`fin`,`total`,`inv_ejercida`,`fact`,`activo`)
                    VALUES ('". $pyin . "','".$descripcion."', STR_TO_DATE('".$fecha."','%d-%m-%Y %H:%i:%s'),'". $nombre . "','". $ramo . "','". $cunr . "', STR_TO_DATE('".$fechaFinal."','%d-%m-%Y %H:%i:%s'), '". $total . "',
                    '". $inv_ejercida . "', STR_TO_DATE('".$fact."','%d-%m-%Y %H:%i:%s'),'S')";
                    
            //STR_TO_DATE('".$fechaEfectiva."','%m-%d-%Y %H:%i:%s')' ";

            $ErrMsg = "No se agregó el registro para <b>".$pyin."</b> del Catálogo Programa Proyecto de Inversión.";
            $TransResult = DB_query($SQL, $db, $ErrMsg);
                //“Se agregó el registro 08 del Catálogo Ramo con éxito.”
            $contenido = "Se agregó el registro <b>".$pyin."</b> del Catálogo Programa Proyecto de Inversión con éxito.";
            $result = true;
        }
        else
        {
           $activo='';
            ...
            $SQL = "UPDATE g_cat_ppi SET activo ='S', descripcion = '$descripcion', nomb= '$nombre', ramo='$ramo', cunr='$cunr',total='$total',inv_ejercida='$inv_ejercida', inicio=STR_TO_DATE('$fecha','%d-%m-%Y %H:%i:%s'),fin= STR_TO_DATE('$fechaFinal','%d-%m-%Y %H:%i:%s'),
        fact= STR_TO_DATE('$fact','%d-%m-%Y %H:%i:%s')
         WHERE `pyin` = '".$pyin."'";
            ...
            $result = true;
                ...
                $contenido = "Ya existe Programa Proyecto de Inversión  <b>".$pyin."</b>";
                $result = true;
            }
//  
        }
...
case 'eliminarpyin':
    ...
    $idpyin = $_POST['pyin'];
    $ErrMsg = "No se encontró el registro <b> ".$idpyin."</b> del Catálogo Programa Proyecto de Inversión.";
    $SQL = "SELECT `pyin` FROM `g_cat_ppi` WHERE `id_nu_pp` = '$idpyin'";
    $TransResult = DB_query($SQL, $db, $ErrMsg);

    $pyin = DB_fetch_array($TransResult)['pyin'];

    $SQL = "UPDATE `g_cat_ppi` SET activo = 'N' WHERE `id_nu_pp` = '$idpyin'";
    //$SQL         = "DELETE FROM g_cat_ppi where pyin='" . $pyin . "'";
    $ErrMsg = "No se eliminó el registro <b> ".$pyin."</b> del Catálogo Programa Proyecto de Inversión.";
    $TransResult = DB_query($SQL, $db, $ErrMsg);
    $contenido ="Se eliminó el registro <b>".$pyin."</b> del Catálogo Programa Proyecto de Inversión con éxito.";
    $result = true;
    break;
...
case 'verificarRamo':
    ...
    $Ramo = $_POST['Ramo'];
    if(!$Ramo){
        $ErrMsg = "No se proporcionó un Ramo.";
        $result = false;
        break;
    }

    $SQL = "SELECT `cve_ramo` FROM `g_cat_ramo` WHERE `cve_ramo` = '$Ramo'";
    $TransResult = DB_query($SQL, $db);

    // comprobación de existencia de la información
    $ErrMsg = "No se encontró el Ramo <b>$Ramo</b>.";
    $contenido = ( DB_num_rows($TransResult) ? true : false );
    $result = true;
    break;
...
case 'verificarUR':
    ...
    $UR = $_POST['UR'];
    if(!$UR){
        $ErrMsg = "No se proporcionó una UR.";
        $result = false;
        break;
    }
    ...
    // comprobación de existencia de la información
    $ErrMsg = "No se encontró la UR <b>$UR</b>.";
    $contenido = ( DB_num_rows($TransResult) ? true : false );
    $result = true;
    break;

case 'verificarIntegridadReferencial':

    $contenido = false;
    $PYIN = $_POST['pyin'];
    if(!$PYIN){
        $ErrMsg = "No se proporcionó el PPI.";
        $result = false;
        break;
    }

    $SQL = "SELECT `pyin` FROM `g_cat_ppi` WHERE `id_nu_pp` = $PYIN";
    $TransResult = DB_query($SQL, $db);
    $myrow = DB_fetch_array($TransResult);

    $SQL = "SELECT * FROM `chartdetailsbudgetbytag` WHERE `pyin` = '$myrow[pyin]'";
    $TransResult = DB_query($SQL, $db);

    // comprobación de existencia de la información
    $ErrMsg = "No se encontró el PPI <b>$PYIN</b>.";
    $contenido = ( DB_num_rows($TransResult) ? true : false );
    $result = true;
    break;

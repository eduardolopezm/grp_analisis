...
define('LNTBLCAT', 'tb_matriz_viaticos');
...
function show($db){
	...
	$info = $_POST['info'];
	$data = ['success'=>false,'msg'=>'No se encontraron datos de la matriz de viáticos','content'=>[]];
...
function store($db){
	...
	$Registro = "<br><strong>$info[ln_clave]</strong></center><p><br>Cuenta Cargo<br><strong>$info[stockact] - $info[stockactTexto]</strong><p><br>Cuenta Abono<br><strong>$info[accountegreso] - $info[accountegresoTexto]</strong>";
	...
	unset($info['txtpp']);
	unset($info['stockactTexto']);
	unset($info['accountegresoTexto']);
...
function compruebaCampoLlave($db, $info)
{
	$sql = "SELECT * 
		FROM `".LNTBLCAT."`
		WHERE `ln_clave` = '".$info['ln_clave']."'
		AND `categoryid` = '".$info['categoryid']."'
		AND `stockact` = '".$info['stockact']."'
		AND `accountegreso` = '".$info['accountegreso']."'";
	$result = DB_query($sql, $db);

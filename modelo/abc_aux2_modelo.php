...
case 'mostrarCatalogo':
    ...
    $info = array();
    $SQL = "SELECT cg,descripcion FROM g_cat_aux2 ".$sqlcg." ORDER BY cg asc";
    $ErrMsg =  "No se obtuvieron registros del Catálogo AUX 2";
    ...
    // Columnas para el GRID
    $columnasNombresGrid .= "[";
    $columnasNombresGrid .= " { text: 'AUX2', datafield: 'CG', width: '5%', cellsalign: 'center', align: 'center', hidden: false },";
...
case 'AgregarCatalogo':
    ...
    if ($proceso == 'Modificar') {
        $info = array();
        $SQL = "UPDATE g_cat_aux2 SET descripcion = '$descripcion'  WHERE cg = '".$cg."'";
        $ErrMsg = "No se modificó el registro para <b>".$cg."</b> del Catálogo Auxiliar 2";
        $TransResult = DB_query($SQL, $db, $ErrMsg);

        $contenido = "Se modificó el registro <b>".$cg."</b> del Catálogo Auxiliar 2 con éxito.";
        $result = true;
    }
    else
    {
        $SQL = "SELECT cg,activo FROM g_cat_aux2 WHERE cg='" . $cg . "'";
        //AND descripcion = '$descripcion'";
        $ErrMsg = "No se obtuvieron registros del Catálogo AUX2";
        $TransResult = DB_query($SQL, $db, $ErrMsg);
        if (DB_num_rows($TransResult) == 0) {
            $info = array();

                    $SQL = "INSERT INTO g_cat_aux2 (`cg`, `descripcion`,`activo`)
                    VALUES ('". $cg . "','".$descripcion."','S')";
                    

            $ErrMsg = "No se agregó la información de ".$cg." - ".$descripcion;
            $TransResult = DB_query($SQL, $db, $ErrMsg);

            $contenido =  "Se agregó el registro <b>".$cg."</b> del Catálogo Auxiliar 2 con éxito.";
            $result = true;
        }
        else
        {
            ...
            if($activo=='N') {
                ...
                $SQL = "UPDATE g_cat_aux2  SET activo ='S',descripcion='".$descripcion."' WHERE cg ='".$cg."'";
                $ErrMsg = "No se agregó el registro para <b>".$cg."</b> del Catálogo Auxiliar 2.";
                $TransResult = DB_query($SQL, $db, $ErrMsg);
                $contenido  ="Se agregó el registro <b>".$cg."</b> del Catálogo Auxiliar 2 con éxito.";
                $result = true;

            }else{
                $contenido = "Ya existe Auxiliar 2 <b>".$cg."</b>";
                $result = true;
            }
        }
    }
...
case 'eliminarcg':
    ...
    $cg = $_POST['cg'];
          $SQL         = "UPDATE  g_cat_aux2  SET activo='N' where cg='".$cg."'";
    $ErrMsg = "No se eliminó el registro <b> ".$cg."</b> del Catálogo Auxiliar 2.";
    $TransResult = DB_query($SQL, $db, $ErrMsg);

    $contenido = "Se eliminó el registro <b>".$cg."</b> del Catálogo Auxiliar 2 con éxito.";
    $result = true;
    break;

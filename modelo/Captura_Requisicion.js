...
$(document).ready(function() {
   ...
   //  setInterval('window_onload()',5000);
...
function fnCargaPartidaArticulos(orden, orderno) {
    ...
    $.ajax({
        ...
        if (data.result) {
            ...
            if (!regresadatos) {
                muestraMensaje("No existen articulos para esa partida seleccionada.");
            }
        }
    }).fail(function(result) {
        ocultaCargandoGeneral();
...
function fnArticulo(noReq,orden,fEntrega,idPartida,idItem,descItem,unidad,cantidad,precio,total,existencia,clavepresupuestal,descLarga,renglon){
    ...
    $(containerArticulo).append('<div class="row p0 m0">'+
            '<ol id=idElementArticulo' + orden + ' class="idElementArticulo col-lg-12 col-md-12 col-sm-12 p0 m0">' +
                ...
                '<li id="idPEArticulo' + orden + '" class="w5p pt5"><input onkeypress="return soloNumeros(event)" class="num w100p addedPEArticulo text-right" type="text" id="addedPEArticulo' + orden + '" placeholder="Precio" onblur="fnValidaPresupuesto('+orden+');" readonly="readonly" disabled="disabled"></li>'+
                '<li id="idCantidadTotalArticulo' + orden + '" class="w5p pt5"><input class="addedCantidadTotalArticulo coin w100p  text-right" type="text" id="addedCantidadTotalArticulo' + orden + '" placeholder="Total" readonly="readonly" disabled="disabled"></li>'+
...
function fnServicio(noReq,orden,fEntrega,idPartida,descPartida,idItem,descItem,precio,clavepresupuestal,descLarga,renglon){
    ...
    $(containerServicio).append('<div class="row p0 m0">' +
        '<ol id=idElementServicio' + orden + ' class="col-lg-12 col-md-12 col-sm-12 p0 m0">' + 
                ...
                '<li id="idAddedReglonServicio' + orden + '" class="w10p pt5"><input onkeypress="return fnSoloBorrar(event)" class="w70p mr2" type="text" id="addedReglonServicio' + orden + '" value="" placeholder="Renglón"/><div id="btnanexorenglonserv'+orden+'" class="w20p mb3 p0 btn btn-default btn-xs glyphicon glyphicon-th-list" onclick="fnMuestraAnexo(' + orden + ');fnMostrarRequisicionModal();"></div></li>'+
                '</ol>' +
            '</div>'+
            '<div id="idCvePresupuestal' + orden + '" class="row borderB ptb5 m0 text-center hide">'+
                '<ol>'+
        ...
        $.ajax({
            ...
            if (data.result) {
                ...
                for (var info in dataJson) {
                    ...
                    contenidoCvePresupuestalServ = dataJson[info].cvePresupuestal;
...
function fnGuardarRequisicion(){
    ...
    //if (fnValida()) {
        if(idReq == ''){
            ...
            if(noReq > 0){
                fnGuardarRequisicionExistente(idReq, comments, fechaFrom, fechaTo, tagref, ue);
            }else{
                fnGuardarRequisicionNueva(idReq, comments, fechaFrom, fechaTo, tagref, ue);
            }
        }
    //}
}
...
function fnValida(){
    ...
    $("#idMainListContentArticulo div[id*=idCvePresupuestal]").each(function(index, el){
        ...
        if ($("#selectCvePartidaEspecifica"+numero).val() == 0) {
            mensaje+= '<p><i class="glyphicon glyphicon-remove-sign text-danger" aria-hidden="true"></i>&nbsp;&nbsp;La partida '+ numero +' no tiene seleccionada partida especifica.</p>';
        }

        if ($("#selectCveArticulo"+numero).val() == 0) {
            mensaje+= '<p><i class="glyphicon glyphicon-remove-sign text-danger" aria-hidden="true"></i>&nbsp;&nbsp;La partida '+ numero +' no tiene seleccionada la clave de artículo.'+"<br>";
        }

        if ($("#selectArticulo"+numero).val() == 0) {
            mensaje+= '<p><i class="glyphicon glyphicon-remove-sign text-danger" aria-hidden="true"></i>&nbsp;&nbsp;La partida '+ numero +' no tiene seleccionada la descripción del artículo.'+"<br>";
        }

        if ($("#addedCantidadArticulo"+numero).val() == 0 || $("#addedCantidadArticulo"+numero).val() == "" || $("#addedCantidadArticulo"+numero).length == 0) {
            mensaje+= '<p><i class="glyphicon glyphicon-remove-sign text-danger" aria-hidden="true"></i>&nbsp;&nbsp;La partida '+ numero +' no tiene capturada la cantidad.'+"<br>";
        }

        if ($("#addedPEArticulo"+numero).val() == 0 || $("#addedPEArticulo"+numero).val() == "" || $("#addedPEArticulo"+numero).length == 0) {
            mensaje+= '<p><i class="glyphicon glyphicon-remove-sign text-danger" aria-hidden="true"></i>&nbsp;&nbsp;La partida '+ numero +' no tiene capturado el precio.'+"<br>";
        }
    });
    ...
    $("#idMainListContentServicio div[id*=idCvePresupuestal]").each(function(index, el){
        ...
        if ($("#selectCvePartidaEspecificaServ"+numero).val() == 0) {
            mensaje+= '<p><i class="glyphicon glyphicon-remove-sign text-danger" aria-hidden="true"></i>&nbsp;&nbsp;La partida '+ numero +' no tiene seleccionada una partida especifica.'+"<br>";            
        }

        if ($("#selectDescPartidaEspecificaServ"+numero).val() == 0) {
            mensaje+= '<p><i class="glyphicon glyphicon-remove-sign text-danger" aria-hidden="true"></i>&nbsp;&nbsp;La partida '+ numero +' no tiene seleccionada la descripción de la partida específica.'+"<br>";
        }

        if ($("#descServicio"+numero).val() == 0 || $("#descServicio"+numero).val() == "" || $("#descServicio"+numero).length == 0) {
            mensaje+= '<p><i class="glyphicon glyphicon-remove-sign text-danger" aria-hidden="true"></i>&nbsp;&nbsp;La partida '+ numero +' no tiene capturada una descripción larga del servicio.'+"<br>";
        }
...
function fnGuardarRequisicionNueva(idReq, comments, fechaFrom, fechaTo, tagref, ue){
...
function fnGuardarRequisicionExistente(idReq,comments,fechaFrom,fechaTo, tagref, ue){
    var status = $("#idStatusReq").val();
...
function fnAgregaRenglon(ordenAnexo,ordenReq,ordenPartida){
    ...
    if (contAnexo == ''){
        ...
        $("#"+ordenAnexo+"_"+ordenPartida+"_"+ordenReq).removeAttr("onclick");
        //$("#"+ordenAnexo+"_"+ordenPartida+"_"+ordenReq).unbind( "click" );
        //$("#"+ordenAnexo+"_"+ordenPartida+"_"+ordenReq).off( "click", "**" );
        //$("#"+ordenAnexo+"_"+ordenPartida+"_"+ordenReq).addClass( "hide" );
        //$("#addedReglonArticulo"+ordenReq).trigger('change');
        fnModificarArticulo(noReq,ordenReq);
    }

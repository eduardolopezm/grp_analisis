 ...
 * Modelo para el proceso del Panel de Programas de Baja Patrimonial
 */
...
if ($option == 'obtenerSificiencia') {
    ...
    if (!empty($tagref) and $tagref != '-1') {
        $sqlWhere .= " AND tb_Fixed_Baja_Patrimonial.sn_tagref IN (".$tagref.") ";
    }
    ...
    if (!empty($fechaDesde) && !empty($fechaHasta)) {
        ...
        $sqlWhere .= " AND tb_Fixed_Baja_Patrimonial.dtm_fecha between '".$fechaDesde." 00:00:00' AND '".$fechaHasta." 23:59:59' ";
    } elseif (!empty($fechaDesde)) {
        $fechaDesde = date_create($fechaDesde);
        $fechaDesde = date_format($fechaDesde, 'Y-m-d');

        $sqlWhere .= " AND tb_Fixed_Baja_Patrimonial.dtm_fecha >= '".$fechaDesde." 00:00:00' ";
    } elseif (!empty($fechaHasta)) {
        $fechaHasta = date_create($fechaHasta);
        $fechaHasta = date_format($fechaHasta, 'Y-m-d');

        $sqlWhere .= " AND tb_Fixed_Baja_Patrimonial.dtm_fecha <= '".$fechaHasta." 23:59:59' ";
    }

    if ($ue != '') {
        $sqlWhere .= " AND (tb_Fixed_Baja_Patrimonial.ln_ue IN (".$ue.") OR tb_Fixed_Baja_Patrimonial.ln_ue IN (".$ue.")) ";
    }

    if ($tipoSuficiencia != '') {
        $sqlWhere .= " AND tb_Fixed_Baja_Patrimonial.nu_tipo_bien IN (".$tipoSuficiencia.") ";
    }

    if ($folio != '') {
        $sqlWhere .= " AND tb_Fixed_Baja_Patrimonial.nu_transno = '".$folio."' ";
    }
    ...
    if ($selectEstatusCompromiso != '') {
        $sqlWhere .= " AND tb_Fixed_Baja_Patrimonial.nu_estatus IN (".$selectEstatusCompromiso.") ";
    }
    ...
    LEFT JOIN tb_Fixed_Baja_Patrimonial_Detalle ON tb_Fixed_Baja_Patrimonial_Detalle.nu_type = tb_Fixed_Baja_Patrimonial.nu_type AND tb_Fixed_Baja_Patrimonial_Detalle.nu_transno = tb_Fixed_Baja_Patrimonial.nu_transno
    LEFT JOIN fixedassets ON fixedassets.assetid = tb_Fixed_Baja_Patrimonial_Detalle.nu_assetid
    JOIN sec_unegsxuser ON sec_unegsxuser.tagref = tb_Fixed_Baja_Patrimonial.sn_tagref AND sec_unegsxuser.userid = '".$_SESSION['UserID']."'
    JOIN tb_sec_users_ue ON tb_sec_users_ue.tagref = tb_Fixed_Baja_Patrimonial.sn_tagref AND tb_sec_users_ue.ue = tb_Fixed_Baja_Patrimonial.ln_ue AND tb_sec_users_ue.userid = '".$_SESSION['UserID']."'
    ...
    WHERE 1 = 1 ".$sqlWhere."
    GROUP BY tb_Fixed_Baja_Patrimonial.nu_type, tb_Fixed_Baja_Patrimonial.nu_transno
    ORDER BY tb_Fixed_Baja_Patrimonial.dtm_fecha DESC
    ";
    ...
    while ($myrow = DB_fetch_array($TransResult)) {
        ...
        $liga= "URL=" . $url;
        $operacion = '<a type="button" id="btnAbrirDetalle_'.$myrow['nu_transno'].'" name="btnAbrirDetalle_'.$myrow['nu_transno'].'" href="bajaPratrimonial.php?'.$liga.'" title="Detalle Pago" style="color: blue;">'.$myrow ['nu_transno'].'</a>'; // target="_blank"
    ...
    $columnasNombresGrid .= " { text: 'Tipo Bien', datafield: 'tipoBien', width: '13%', editable: false, cellsalign: 'center', align: 'center', hidden: false },";
    $columnasNombresGrid .= " { text: 'Justificación', datafield: 'sn_description', width: '30%', editable: false, cellsalign: 'center', align: 'center', hidden: false },";
...
if ($option == 'actualizarEstatus') {
    ...
    foreach ($dataJsonNoCapturaSeleccionados as $datosClave) {
        ...
        WHERE tb_Fixed_Baja_Patrimonial.nu_type = '".$datosClave ['type']."' AND tb_Fixed_Baja_Patrimonial.nu_transno = '".$datosClave ['transno']."'";
        ...
        if (($estatusActual == '0' || $estatusActual == '4')) {
            ...
            $mensajeInfo .= '<p><i class="glyphicon glyphicon-ok-sign text-danger" aria-hidden="true"></i> Actualización para la Baja Patrimonial con Folio '.$datosClave['transno'].' no es posible en el estatus que se encuentra ('.$nombreEstatus.')</p>';
            ...
            $mensajeInfo .= '<p><i class="glyphicon glyphicon-ok-sign text-danger" aria-hidden="true"></i> La Baja Patrimonial con Folio '.$datosClave['transno'].' ya se encuentra en '.$nombreEstatus.'</p>';
        } else {
            $statusActualizacion = $statusid;
            if ($statusid == '99') {
                // Obtener Estatus Anterior
                $statusActualizacion = $statusAnterior;
            }

            if ($statusActualizacion == '0') {
                // Si es cancelacion regresar a disponible los activos
                $SQL = "UPDATE fixedassets
                JOIN tb_Fixed_Baja_Patrimonial_Detalle ON tb_Fixed_Baja_Patrimonial_Detalle.nu_assetid = fixedassets.assetid 
                AND tb_Fixed_Baja_Patrimonial_Detalle.nu_type = '".$datosClave ['type']."'
                AND tb_Fixed_Baja_Patrimonial_Detalle.nu_transno = '".$datosClave ['transno']."'
                SET fixedassets.status = '1'";
                $ErrMsg = "No se actualizo los registros a disponible";
                $TransResult = DB_query($SQL, $db, $ErrMsg);
            }

            // Actualizar estatus tabla principal
            $SQL = "UPDATE tb_Fixed_Baja_Patrimonial SET tb_Fixed_Baja_Patrimonial.nu_estatus = '".$statusActualizacion."'
            WHERE tb_Fixed_Baja_Patrimonial.nu_type = '".$datosClave ['type']."' AND tb_Fixed_Baja_Patrimonial.nu_transno = '".$datosClave ['transno']."'";
            ...
            if (empty($msjConfigurado)) {
                $mensajeInfo .= '<p><i class="glyphicon glyphicon-ok-sign text-success" aria-hidden="true"></i> Actualización para la Baja Patrimonial con Folio '.$datosClave['transno'].' correcta</p>';
            } else {

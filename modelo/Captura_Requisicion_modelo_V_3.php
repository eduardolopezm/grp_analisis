...
if ($option == 'guardarRequisicion') {
    ...
    if ($status == 'Original' || $status == 'Autorizado' || $status == 'Cancelado') {
        ...
        if ($error > 0) {
            ...
            $SQL= "UPDATE purchorders SET status = '$status', comments = '$comments', validfrom = '$fechaFrom', validto = '$fechaTo', deliverydate = '$fechaTo', fecha_modificacion = current_timestamp(), tagref= '$tagref', nu_ue= '$ue', nu_anexo_tecnico = '$anexoTec', ln_codigo_expediente = '$codigoExpediente', ln_UsoCFDI = '$selectCFDI'
            WHERE orderno  = '$req'";

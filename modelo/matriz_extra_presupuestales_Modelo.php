...
define('LNTBLCAT', 'tb_matriz_extraptal');
...
define('DATA', [
	...
	'wipact' => ['col' => 'wipact', 'tipo' => 'integer'],
	'allowNarrativePOLine' => ['col' => 'allowNarrativePOLine', 'tipo' => 'integer'],
		'ordendesplegar' => ['col' => 'ordendesplegar', 'tipo' => 'string'],
		'factesquemadoancho' => ['col' => 'factesquemadoancho', 'tipo' => 'integer'],
		'factesquemadoalto' => ['col' => 'factesquemadoalto', 'tipo' => 'integer'],
	'accounttransfer' => ['col' => 'accounttransfer', 'tipo' => 'integer'],
	...
	'idflujo' => ['col' => 'idflujo', 'tipo' => 'integer'],
	'cashdiscount' => ['col' => 'cashdiscount', 'tipo' => 'integer']
]);
...
function show($db){
	...
	$sql = "SELECT categoryid,
				...
				stockcategory.nu_tipo_gasto, ln_concepto, ln_documento,
				tb_matriz_extraptal_origen.txt_descripcion AS descripcionOrigen,
				tb_matriz_extraptal_proceso.txt_descripcion AS descripcionProceso,
				stockcategory.ordendesplegar,
			...
			LEFT JOIN tb_matriz_extraptal_origen ON tb_matriz_extraptal_origen.nu_reg = stockcategory.factesquemadoancho
			LEFT JOIN tb_matriz_extraptal_proceso ON tb_matriz_extraptal_proceso.nu_reg = stockcategory.factesquemadoalto
	...
	if(empty($info['busquedaClav'])==false){
		$sql .= " AND stockcategory.ordendesplegar LIKE '%" . $info['busquedaClav'] . "%' ";
	}
	...
	while ($rs = DB_fetch_array($result)) {
		$data['content'][] = [
			...
			'ordendesplegar'=>utf8_encode($rs['ordendesplegar']),// 2
			'descripcionOrigen'=>utf8_encode($rs['descripcionOrigen']),// 3
			'descripcionProceso'=>utf8_encode($rs['descripcionProceso']),// 4
			'categorydescription'=>utf8_encode($rs['categorydescription']),// 5
			'nu_tipo_gasto'=>utf8_encode($rs['nu_tipo_gasto']),// 6
			'stockact'=>utf8_encode($rs['stockact']),// 7
			'nombreCargo'=>utf8_encode($rs['nombreCargo']),// 8
			'accountegreso'=>utf8_encode($rs['accountegreso']),// 9
			'nombreAbono'=>utf8_encode($rs['nombreAbono']),// 10
			'estatus'=>utf8_encode($rs['Estatus']),// 11
			'modificar'=>'<span class="modificar glyphicon glyphicon-edit"></span>',// 12
			'eliminar'=>'<span class="eliminar glyphicon glyphicon-trash"></span>',// 13
			'identificador'=>$rs['id']// 14
		];
...
function store($db){
	...
	if(compruebaCampoLlave($db, $info) && !$valid){
		$data['msg'] = "El registro:".str_replace("<p><br>", "<p>", $Registro)."<p>ya existe en el sistema.";
...
function compruebaCampoLlave($db, $info){
	...
	$sql = "SELECT * 
		FROM `".LNTBLCAT."`
		WHERE `ln_clave` LIKE '$info[ln_clave]'
		AND `categoryid` LIKE '$info[categoryid]'
...
function datosselectOrigen($db){
	$sql = "SELECT nu_reg AS valor, txt_descripcion AS label
			FROM tb_matriz_extraptal_origen
			WHERE sn_activo = '1'";
...
function datosListaCuentaAbono($db){
	$sql = "SELECT DISTINCT `accountcode` AS valor, `accountname` AS label 
			...
			AND LENGTH(`accountcode`) >= 7
			ORDER BY `accountcode`";
	
	return obtenDatosLista($db,$sql);
}

function datosselectProceso($db){
	$sql = "SELECT nu_reg AS valor, txt_descripcion AS label
			FROM tb_matriz_extraptal_proceso
			WHERE sn_activo = '1'";
	
	return obtenDatosSelect($db,$sql);
}

function datosselectTipoPrograma($db){
	$sql = "SELECT `cpp`.`cppt` AS valor, 'pp' AS `label`
                FROM `tb_cat_programa_presupuestario` AS `cpp`
                INNER JOIN `g_cat_ramo` AS cr on `cpp`.`cve_ramo` = `cr`.`cve_ramo`

                WHERE `activo` = '1'
            UNION
                SELECT `pe` AS `valor`, 'pe' AS `label`

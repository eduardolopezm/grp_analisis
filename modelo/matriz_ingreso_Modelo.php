...
define('LNTBLCAT', 'tb_matriz_ingreso');
...
define('DATA', [
	...
	'txtPartidaEspecifica'=>['col'=> 'partida', 'tipo'=> 'string'],
	'selectCapitulo'=>['col'=> 'ln_capitulo', 'tipo'=> 'string']
...
function show($db){
	...
	$sql = "SELECT categoryid,
			...
			WHERE stocktype<>'A'";
	//LEFT JOIN tb_cat_partidaspresupuestales_capitulo tb_capitulo on 
...
function store($db){
	...
	/*
	*Se agregó validacion de la ue para la ministracion 
	*que segnera por la ue 00-oficina central
	*/
	if($info['selectUnidadEjecutora'] == '-1'){
		$info['selectUnidadEjecutora'] = "00";
	}

	//$info['ln_clave'] = $info['selectUnidadNegocio']."-".$info['selectUnidadEjecutora']."-".$info['txtpp'];

	if($info['selectCapitulo'] != "" && $info['txtpp'] != "-1"){

		$SQL="SELECT ccapmiles FROM tb_cat_partidaspresupuestales_capitulo where ccap = '".$info['selectCapitulo']."'";
		$result = DB_query($SQL,$db);
		$myrow = DB_fetch_array($result);
		$info['ln_clave'] = $info['selectUnidadNegocio']."-".$info['selectUnidadEjecutora']."-".$myrow['ccapmiles'];
	}else{
		$info['ln_clave'] = $info['selectUnidadNegocio']."-".$info['selectUnidadEjecutora']."-".$info['txtpp'];
	}

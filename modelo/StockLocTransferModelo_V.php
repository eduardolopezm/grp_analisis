 ...
 * @category ABC
 * @package ap_grp
 * @author       Japheth Calzada <[<email address>]>
 * @license      [<url>] [name]
 * @version      GIT: [<description>]
 * @link(target, link)
 * Fecha Creación: 27/08/2018
 * Fecha Modificación: 02/08/201
 * Se realizan operación de captura de la salida de transferencia de almacén.
 */
//////
...
if ($option == 'loadSalida') {
    $reference = $_POST['reference'];
    $info = array();
    $SQL="SELECT idtransfer,loctransfers.stockid,shipqty,shiploc,recloc,comments,
            ( SELECT partidaEspecifica FROM tb_partida_articulo WHERE tb_partida_articulo.eq_stockid = stockmaster.eq_stockid) AS partida
            FROM loctransfers JOIN stockmaster ON loctransfers.stockid=stockmaster.stockid
            WHERE  reference = $reference";

    $ErrMsg = "No se encontro la salida: ".$reference;
    $TransResult = DB_query($SQL, $db, $ErrMsg);
...
if ($option == 'mostrarAlmacen') {
    $sql = "SELECT locations.loccode as value, CONCAT(locations.loccode,' - ',locations.locationname) as texto
...
if ($option == 'mostrarCppt') {
    ...
    $SQL = "SELECT DISTINCT chartdetailsbudgetbytag.accountcode as cvefrom, tagref, cppt, ln_aux1,tb_cat_unidades_ejecutoras.ue as ue, concat(tagref,'-',tb_cat_unidades_ejecutoras.ue,'-',cppt) as diferenciador 
    FROM chartdetailsbudgetbytag 
    JOIN tb_cat_unidades_ejecutoras ON tb_cat_unidades_ejecutoras.ln_aux1 = chartdetailsbudgetbytag.ln_aux1
    WHERE chartdetailsbudgetbytag.tagref='$tagref' 
    AND chartdetailsbudgetbytag.partida_esp= '$cvePA' 
...
if ($option == 'mostrarPartidaCvePptoProdct') {
    ...
    $SQL= "SELECT stockmaster.stockid AS idProducto, 
            ...
            LEFT JOIN (SELECT locstock.loccode, locstock.stockid, locstock.quantity
                        ...
                        AND sec_loccxusser.userid= '".$_SESSION['UserID']."') existencia ON stockmaster.stockid= existencia.stockid
    ...
    INNER JOIN tb_partida_articulo on (stockmaster.eq_stockid = tb_partida_articulo.eq_stockid )
    LEFT JOIN  tb_cat_partidaspresupuestales_partidaespecifica on (tb_cat_partidaspresupuestales_partidaespecifica.partidacalculada = tb_partida_articulo.partidaEspecifica)
    LEFT JOIN stockcostsxlegal ultimo_costo ON stockmaster.stockid = ultimo_costo.stockid AND ultimo_costo.legalid = (SELECT legalid FROM tags WHERE tagref = '".$datoTagref."')
    LEFT JOIN (
...
if ($option == 'mostrarPartidaCveServicio') {
    ...
    //         stockmaster.description AS descPartidaEspecifica,
    //         stockmaster.stockid AS idServicio,
    //         stockmaster.description AS descripcionServicio,
    //         IF(stockcostsxlegal.lastcost >= 0 ,stockcostsxlegal.lastcost,'0')  AS precioEstimado
    //         FROM stockmaster
    ...
    //         AND stockmaster.mbflag = 'D'
    //         AND tb_partida_articulo.partidaEspecifica <> ''";
...
if ($option == 'agregarElementosRequisicion') {
    ...
    VALUES 
    (
        '$numRequisicion','no_data','$deliverydate','descripcion','1.1.5.1.1','0','100','100','100','10','0',0,0,0,'','',0,'',0,0,'',0,0,'100','1000',0,0,0,'','',0,current_timestamp(),0,0,'$ordenElemento',0,0,0,'','',0,'','','','1900-01-01','1900-01-01',current_timestamp(),'',1,1,'0','$cvePresupuestal','descLarga','', 0, '$cppt'
    )";
...
if ($option == 'modificarElementosSalida') {
    $claveArticulo          = $_POST["claveArticulo"];
    $descArticulo           = $_POST["descArticulo"];
    $addedCantidadArticulo  = $_POST["addedCantidadArticulo"];
    $addObservaciones       = $_POST["addObservaciones"];
    $idTransfer             = $_POST["idTransfer"];
    $reference              = $_POST[" reference"];
    $hoy                    = date("Y-m-d H:m:s");
    $updateObservaciones    = '';
    if ($addObservaciones != '') {
        $updateObservaciones = " ,comments='$addObservaciones' ";
    }
    $SQL="UPDATE loctransfers SET stockid=$claveArticulo, shipqty=$addedCantidadArticulo,updatedate='$hoy' $updateObservaciones WHERE idtransfer = $idTransfer ";
    $ErrMsg = "No se pudo modificar la Salida: ".$req;
    $TransResult = DB_query($SQL, $db, $ErrMsg);
    
    $contenido = "Se Modifico la Requisición: ".$reference;
    if ($TransResult) {
        $result = true;
    }
}

if ($option == 'eliminarElementos') {
    $result         = false;
    $idTransfer     = $_POST['idTransfer'];
    $info           = array();

    $SQL = "DELETE FROM loctransfers WHERE idtransfer = $idTransfer";
...
if ($option == 'obtenerFolio') {
    $SQL = "SELECT IFNULL(MAX(`reference`),0)+1 AS `reference` FROM `loctransfers`";
    $TransResult = DB_query($SQL, $db, $ErrMsg);
    while ($myrow = DB_fetch_array($TransResult)) {
        $info[] = array(
...
if ($option == 'validarExistencia') {
    $cveArticulo            = $_POST["cveArticulo"];
    $cantidadArticulo       = $_POST["cantidadArticulo"];
    $almacenRem             = $_POST["almacenRem"];
    $SQL="SELECT SUM(qty)-$cantidadArticulo as existencia FROM stockmoves";
    $SQL.=' WHERE stockid="'.$cveArticulo .'"';
    $SQL.=' AND loccode="'.$almacenRem.'" AND type <> 1001';
...
if ($option == 'guardarTransferencia') {
    $almacenRem         = $_POST['almacenRem'];
    $status             = $_POST['status'];
    $almacenDes         = $_POST['almacenDes'];
    $partidaEspecifica  = $_POST['partidaEspecifica'];
    $cveArticulo        = $_POST['cveArticulo'];
    $descArticulo       = $_POST['descArticulo'];
    $cantidadArticulo   = $_POST['cantidadArticulo'];
    $addObservaciones   = $_POST['addObservaciones'];
    ...
    $SQL = "INSERT INTO 
                loctransfers (reference,stockid,shipqty, registerdate, shiploc,recloc,comments, userregister,statustransfer)
                VALUES  ($folio, $cveArticulo,$cantidadArticulo,'$hoy',$almacenRem,$almacenDes,'$addObservaciones','$usuariologueado','Capturado') ";
    $numero ++;
    //echo "$numero ->";
    $ErrMsg = "No se obtuvieron los botones para el proceso";
    $TransResult = DB_query($SQL, $db, $ErrMsg);

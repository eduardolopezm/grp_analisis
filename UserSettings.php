...
echo '<p class="page_title_text"><img src="'.$rootpath.'/css/'.$theme.'/images/user.png" title="' . _('User Settings') . '" alt="">' . ' ' . _('Configuración de Usuarios');
 ...
 */
	...
	if ($InputError != 1) {
	  ...
	  // update the session variables to reflect user changes on-the-fly
		...
		$_SESSION['ShowIndex']=$_POST['ShowIndex'];
...
echo '<tr><td>' . _('Opcion Menu') . ":</td><td><select name='ShowIndex'>";
...
echo '	<tr><td>' . _('Nuevo Password') . ":</td>
	<td><input type='password' name='pass' size=20 value='" .  $_POST['pass'] . "'></td></tr>
	<tr><td>" . _('Confirma Password') . ":</td>
	<td><input type='password' name='passcheck' size=20  value='" . $_POST['passcheck'] . "'></td></tr>
	<tr><td colspan=2 align='center'><i>" . _('Si dejas alguna de las cajas de texto vacías, el password no se modificará') . '</i></td></tr>
	<tr><td>' . _('Email') . ':</td>';
...
echo "<td><input type=text name='email' size=40 value='" . $_POST['email'] . "'></td></tr>
	...
	<br><div class='centre'><input type='Submit' name='Modify' value=" . _('Modificar') . '></div>
	</form>';

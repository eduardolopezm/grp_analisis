...
$jreport = $JasperReport->compilerReport("compromiso");

$sqllogo = "SELECT legalbusinessunit.logo
FROM tb_compromiso
INNER JOIN tags ON tags.tagref = tb_compromiso.sn_tagref
INNER JOIN legalbusinessunit ON tags.legalid = legalbusinessunit.legalid
WHERE tb_compromiso.nu_type = '".$_GET["type"]."' AND tb_compromiso.nu_transno = '".$_GET["transno"]."'";

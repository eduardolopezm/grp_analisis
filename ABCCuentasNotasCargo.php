...
//ABC Cuentas para Notas Cargo de Proveedor
$funcion=251;
$PageSecurity = 3;
...
$men=$_GET['menu'];
	...
	if ($listado == "1"){
		...
		$sql="select * from chartbridgecargos_supp";
		$res=(db_query($sql,$db));
		}
	...
	if ($men == "2" ){
		...
		$sql="select * from chartbridgecargos_supp where accountcode='" . $_GET['cuenta'] . "'";
		$res=(db_query($sql,$db));
	...
	if ($ok_process == true && $_POST['boton'] == 'Procesar' ){
			...
			$sql="select count(*) as cuenta from chartbridgecargos_supp where accountcode='". $_POST['cuenta1'] ."'";
			$res=(db_query($sql,$db));
			$myrow=db_fetch_row($res);
		
			if ($myrow[0]>0){
				echo " <b> <font color=#FF0000> Error ya existe la cuenta: </font> </b><b>" .$_POST['cuenta1']."</b>";
				echo "<center <a href='ABCCuentasNotasCargo.php?men=1' > REGRESAR </a> </center>";

				}
			else {
				$sql="insert into chartbridgecargos_supp(accountcode,concept)";
			...
			$sql="select count(*) as cuenta from chartbridgecargos_supp where accountcode like '%". $_POST['cuenta1'] ."%'";
			$res=(db_query($sql,$db));
			$myrow=db_fetch_row($res);
			if ($myrow[0]<=0){
				...
				$sql="select * from chartbridgecargos_supp where accountcode like '%".$_POST['cuenta1']."%'";
				$res=(db_query($sql,$db));
				$listado="2";
				echo "<br>";
				echo "<br>";
			...
			$sql="UPDATE chartbridgecargos_supp SET concept='$_POST[concepto]' WHERE accountcode='$myrow[accountcode]'";
			$res=(db_query($sql,$db));
			$listado="1";
			$men="1";
			$sql="select * from chartbridgecargos_supp";
			...
			echo"<META HTTP-EQUIV='refresh' CONTENT='2; URL=ABCCuentasNotasCargo.php?men=1'>";
			
	}elseif($_POST['botsi'] == 'ELIMINAR'){
			$sql="DELETE FROM chartbridgecargos_supp WHERE accountcode='$myrow[accountcode]'";
			$res=(db_query($sql,$db));
			$listado="1";
			$men="1";
			$sql="select * from chartbridgecargos_supp";
...
if ($men != "2" && $men != "3"){
	...
	echo "<br>";
		...
		echo '<p class="page_title_text"><img src="'.$rootpath.'/css/'.$theme.'/images/note_add.png" title="' . _('CUENTAS NOTAS DE CARGO') . '" alt="">' . ' ' ." ABC CUENTAS NOTAS DE CARGO </p>";
	...
	echo "<tr>";
		...
		echo "</td>";
		echo "<td>";
			echo "<a href='ABCCuentasNotasCargo.php?menu=2&cuenta=" . $myrow['accountcode'] . "'> Editar </a>";
		echo "</td>";
		
		echo "<td>";
			echo "<a href='ABCCuentasNotasCargo.php?menu=3&cuenta=" . $myrow['accountcode'] . "'> Eliminar </a>";
		echo "</td>";

...
function esUltimoConcepto($index, &$arraycadena) {
	$cadIndex = 0;
	for ($cad = 4; $cad <= count($arraycadena) - 1; $cad++) {
		$linea = $arraycadena[$cad];
		$datos = explode('|', $linea);
...
function esUltimoImpuestoTraslaso($index, &$arraycadena) {
	$cadIndex = 0;
	for ($cad = 4; $cad <= count($arraycadena) - 1; $cad++) {
		$linea = $arraycadena[$cad];
		$datos = explode('|', $linea);
		if ($cad >= 4 AND $datos[0] == '7') {
			$cadIndex = $cad;
		}
	}
...
function sumarCentavoConcepto(&$arraycadena, $carpeta, $nDecimales) {
	$subtotal = 0;
	$subtotalConceptos = 0;
	$centavo = 0.01;
	for ($cad = 4; $cad <= count($arraycadena) - 1; $cad++) {
		...
		if ($cad >= 4 AND $datos[0] == '5') {
			if ($carpeta == 'Recibo') {
				$importe = $datos[6];
			} else if ($carpeta == 'NCargo' OR $carpeta == 'NCreditoDirect') {
				$importe = $datos[13];
			} else {
			...
			$subtotal += $importe;
			$subtotalConceptos += number_format($importe, $nDecimales, '.', '');
		}
	}
	// Cast to numbers
	$subtotalConceptos = $subtotalConceptos * 1;
	$subtotal = number_format($subtotal, $nDecimales, '.', '') * 1;
	if ($subtotal != $subtotalConceptos) {
		$subtotalConceptos = $subtotalConceptos + $centavo;
		// Compare two precision numbers, 0 means equality ...
		if (bccomp($subtotalConceptos, $subtotal, $nDecimales) == 0) {
			return true;
		}
	}
	return false;
...
function sumarCentavoImpuestoTraslaso(&$arraycadena, $nDecimales) {
	$total = 0;
	$totalImpuestos = 0;
	$centavo = 0.01;
	for ($cad = 4; $cad <= count($arraycadena) - 1; $cad++) {
		...
		if ($cad >= 4 AND $datos[0] == '7') {
			$importe = $datos[3];
			$total += $importe;
			$totalImpuestos += number_format($importe, $nDecimales, '.', '');
		}
	}
	// Cast to numbers
	$totalImpuestos = $totalImpuestos * 1;
	$total = number_format($total, $nDecimales, '.', '') * 1;
	if ($total != $totalImpuestos) {
		$totalImpuestos = $totalImpuestos + $centavo;
		// Compare two precision numbers, 0 means equality ...
		if (bccomp($totalImpuestos, $total, $nDecimales) == 0) {
			return true;
		}
	}
	return false;
...
function generaXMLCFDI($cadena_original,$tipocomprobante,$tagref,$serie,$folio,$iddocto,$carpeta,$orderno=0,$db) {
	...
	$totalimporte=number_format($totalimporte,$nDecimales,'.','');
	for($cad=0;$cad<=count($arraycadena)-1;$cad++){
		$linea=$arraycadena[$cad];
		$datos=explode('|',$linea);//
		...
		if ($cad==0){
			...
			$Noaprobacion=$aprobacionfolios[1];
			$anioAprobacion=$aprobacionfolios[2];
			$descuentofact = number_format($datos[9],$nDecimales,'.','');
			if(empty($descuentofact)) {
				$descuentofact = 0;
			}
			...
			"subTotal"=>$totalimporte,
			"descuento"=>$descuentofact,
			"total"=>number_format(($totalimporte-$descuentofact)+$impuestosinifact,$nDecimales,'.',''),
			"metodoDePago"=>$datosdos[3],
			"TipoCambio"=>$datos[13],
			"Moneda"=>$datos[12],
			"LugarExpedicion"=>$myrowtags['municipioexpedido'].','.$myrowtags['estadoexpedido'],
			...
			$cadenasellar=$cadenasellar."|".trim(ReglasXCadena($datos[13]));
			$cadenasellar=$cadenasellar."|".trim(ReglasXCadena($datos[12]));
			$cadenasellar=$cadenasellar."|".ReglasXCadena(number_format(($totalimporte-$descuentofact)+$impuestosinifact,$nDecimales,'.',''));
			$cadenasellar=$cadenasellar."|".trim(ReglasXCadena($datosdos[3]));
			$cadenasellar=$cadenasellar."|".trim(ReglasXCadena($myrowtags['municipioexpedido'].','.$myrowtags['estadoexpedido']));
			$cadenasellar=$cadenasellar."|".ReglasXCadena($datosdos[5]);
			//$cadenasellar=$cadenasellar."|".trim(ReglasXCadena($datos[13]));
			...
			if (esUltimoConcepto($cad, $arraycadena)) {
				if (sumarCentavoConcepto($arraycadena, $carpeta, $nDecimales)) {
					$importe += $centavo;
				}
			}
			...
			"noIdentificacion"=>trim($datos[2]),
			"descripcion"=>trim($datos[4]),
			"valorUnitario"=>trim(number_format($datos[5], $nDecimales, '.', '')),
			"importe"=>trim(number_format($importe, $nDecimales, '.', ''))
			)
			);
			$cadenasellar=$cadenasellar."|".trim(ReglasXCadena($datos[3]))."|".trim(ReglasXCadena($unidades));
			$cadenasellar=$cadenasellar."|".trim(ReglasXCadena($datos[2]))."|".trim(ReglasXCadena($datos[4]));
			//echo $cadenasellar;
			$totalimporte=$totalimporte+$importe;
			$cadenasellar=$cadenasellar."|".trim(ReglasXCadena(number_format($datos[5], $nDecimales, '.', '')))."|".trim(ReglasXCadena(number_format($importe, $nDecimales, '.', '')));
			...
			if (esUltimoImpuestoTraslaso($cad, $arraycadena)) {
				if (sumarCentavoImpuestoTraslaso($arraycadena, $nDecimales)) {
					$datos[3] = number_format($datos[3], $nDecimales, '.', '') + $centavo;
				}
			}
			...
			cargaAtt($traslado, array("impuesto"=>trim($datos[1]),
			"tasa"=>trim($datos[2]),
			"importe"=>trim(number_format($datos[3], $nDecimales, '.', ''))
			)
			);

			$cadenasellarimp=$cadenasellarimp."|".trim(ReglasXCadena($datos[1]))."|".trim(ReglasXCadena($datos[2]))."|".trim(ReglasXCadena(number_format($datos[3], $nDecimales, '.', '')));
		}elseif($cad>=4 and $datos[0]=='6'){
			...
			cargaAtt($retenido, array("impuesto"=>trim($datos[1]),
							"importe"=>trim(number_format(abs($datos[3]), $nDecimales, '.', '')))
			);

			$impuestoretenido=$impuestoretenido+trim($datos[3]);
			$cadenasellar=$cadenasellar."|".trim(ReglasXCadena($datos[1]))."|".trim(ReglasXCadena(number_format(abs($datos[3]), $nDecimales, '.', '')));
			...
			while ($ind<count($datos)){
				...
				if ($cant < 0 ){
					...
					"Importe"=>number_format(abs($cant),$nDecimales)
					)
					);
				}
				else{
					...
					"TasadeTraslado"=>$porc,
					"Importe"=>number_format($cant,$nDecimales)
					)
					);
				}
				$cadimp.="|".$desc."|".$porc."|".number_format(abs($cant),$nDecimales);
			
			
				$ind+=3;
			}
			cargaAtt($impLocal,array("version"=>"1.0",
			"TotaldeRetenciones"=>number_format($tRet,$nDecimales),
			"TotaldeTraslados"=>number_format($tTras,$nDecimales)
			)
			);
			
			$cadimp = utf8_encode("|1.0|".number_format($tRet,$nDecimales)."|".number_format($tTras,$nDecimales).trim($cadimp));
		}
	...
	if ($banderaimpuestosretenidos==true){
		$impuestos->SetAttribute("totalImpuestosRetenidos",number_format(abs($impuestoretenido), $nDecimales, '.', ''));
	}
	...
	if ($banderaimpuestosretenidos==false){
		...
		$cadenasellar=$cadenasellar."|".ReglasXCadena(number_format(abs($impuestoretenido), $nDecimales, '.', ''));//."||";
	}
	
	if ($banderaimpuestos==true){
		$impuestos->SetAttribute("totalImpuestosTrasladados",number_format($impuestofact, $nDecimales, '.', ''));
	}else{
		$impuestos = $xml->createElement("cfdi:Impuestos");
		$impuestos = $root->appendChild($impuestos);
	}
	...
	if ($banderaimpuestos==false){
		...
		$cadenasellar=$cadenasellar.$cadenasellarimp."|".ReglasXCadena(number_format($impuestofact, $nDecimales, '.', ''));//."||";
	}

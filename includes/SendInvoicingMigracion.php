...
function generaXMLIntermedio($txtinput,$xml,$cadenaOriginal,$cantidadLetra,$orderNo,$db,$tipofacturacionxtag,$tagref,$status){
	...
	//Atributo para mostrar etiqueta de cancelado en impresion
	$statusdoc=$domXml->createAttribute("status");
	$statusdoc->value="$status";
	$comprobante->item(0)->appendChild($statusdoc);
	...
	$arraycadena=explode('@%',$input);
	$noConcep=0;
	for($line=0;$line<=count($arraycadena)-1;$line++){
	...
	$SQL="Select Titulo,Texto,consulta,noColumns from PDFTemplates where tipodocto=1";//.$tipofacturacionxtag;
	...
	if (DB_num_rows($Result)>0) {
		while($row=DB_fetch_array($Result)){
			...
			if($query!=null && !empty($query)){
				...
				if(($row['Titulo'] != 'InformacionSucursales') && ($row['Titulo'] != 'InformacionBancaria')){
					$query .= $orderNo;
				}
	...
	echo '<pre>'.(htmlentities($xmlImpresion));

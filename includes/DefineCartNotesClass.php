...
Class Cart {
	...
	function add_to_cart($StockID,
				...
				$DiscPercent2=0,
				$warranty=0,
				$refundperc=1  //porcentaje de devolucion admitido 1 = 100% por default
				){
	...
	function update_cart_item( $UpdateLineNumber, $Qty, $Price, $Disc, $Narrative, $UpdateDB='No', $ItemDue, $POLine,$DiscPercent1,$DiscPercent2,$AlmacenStock,$warranty){
		...
		if ($UpdateDB=='Yes'){
			global $db;
			$result = DB_query("UPDATE notesorderdetails
						SET quantity=" . $Qty . ",
						unitprice=" . $Price . ",
	...
	function remove_from_cart($LineNumber, $UpdateDB='No'){
		...
		if ($UpdateDB=='Yes'){
			...
			if ($this->Some_Already_Delivered($LineNumber)==0){
				...
				$result = DB_query('DELETE FROM notesorderdetails
						     WHERE orderno=' . $_SESSION['ExistingOrder'] . '
				...
				$result = DB_query('UPDATE notesorderdetails SET quantity=qtyinvoiced, completed=1

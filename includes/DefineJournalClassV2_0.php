...
Class Journal {
	...
	function Add_To_GLAnalysis($Amount, $Narrative, $GLCode, $GLActName, $tag, $GLLegalid, $GLLegalName){
		if (isset($GLCode) AND $Amount!=0){
			$this->GLEntries[$this->GLItemID] = new JournalGLAnalysis($Amount, $Narrative, $this->GLItemID, $GLCode, $GLActName, $tag, $GLLegalid, $GLLegalName);
			$this->GLItemCounter++;
			$this->GLItemID++;
...
Class JournalGLAnalysis {
	...
	function JournalGLAnalysis ($Amt, $Narr, $id, $GLCode, $GLActName, $tag, $GLLegalid, $GLLegalName){
...
/* Constructor function to add a new JournalGLAnalysis object with passed params */
		...
		$this->ID = $id;
		$this->tag = $tag;
		$this->GLLegalid = $GLLegalid;
		$this->GLLegalName = $GLLegalName;
	}
}

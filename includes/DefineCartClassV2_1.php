...
Class Cart {
	...
	function setGroupItems(){
		...
		$sql = "Select * FROM salesorderdetailsgroups WHERE orderno = ".$_SESSION['ExistingOrder']." Order By groupname";
		$rs = DB_query($sql,$db);
		...
		if (DB_num_rows($rs) > 0){
			...
			$qry = "Delete from salesorderdetails WHERE orderno = ".$_SESSION['ExistingOrder'];
			$r = DB_query($qry,$db);
				...
				if($row = DB_fetch_array($result)) {
					...
					//echo "<pre>entre 3 .....";
					 ...
					 if($borrarpartida==1){
						...
						prnMsg( _('Se elimino la partida'). ' ' . /*$LineNumber .*/ ' ' . _('de la orden numero').' ' . $_SESSION['ExistingOrder'], 'success');
					}

...
function AgregaAddendaXML($xmlSat,$debtorno,$iddocto,$db){
	...
	$comprobante = $xmlPath->query("/cfdi:Comprobante");
	$emisor = $xmlPath->query("/cfdi:Comprobante/cfdi:Receptor");
...
function generaXMLCFDI($cadena_original,$tipocomprobante,$tagref,$serie,$folio,$iddocto,$carpeta,$orderno=0,$db) {
	...
	$totalimporte=FormatNumberERP($totalimporte);
	for($cad=0;$cad<=count($arraycadena)-1;$cad++){
		$linea=$arraycadena[$cad];
		$datos=explode('|',$linea);
		...
		if ($cad==0){
			...
			$Noaprobacion=$aprobacionfolios[1];
			$anioAprobacion=$aprobacionfolios[2];
			$descuentofact = FormatNumberERP($datos[9]);
			if(empty($descuentofact)) {
				$descuentofact = 0;
			}
			echo "<br/>$totalimporte-$descuentofact+$impuestosinifact<br />";
			...
			"subTotal"=>$totalimporte,
			"descuento"=>$descuentofact,
			"total"=>FormatNumberERP(($totalimporte-$descuentofact)+$impuestosinifact),
			"metodoDePago"=>$datosdos[3],
			"TipoCambio"=>$datos[13],
			"Moneda"=>$datos[12],
			"LugarExpedicion"=>$myrowtags['municipioexpedido'].','.$myrowtags['estadoexpedido'],
			...
			$cadenasellar=$cadenasellar."|".trim(ReglasXCadena($datos[13]));
			$cadenasellar=$cadenasellar."|".trim(ReglasXCadena($datos[12]));
			$cadenasellar=$cadenasellar."|".ReglasXCadena(FormatNumberERP(($totalimporte-$descuentofact)+$impuestosinifact));
			$cadenasellar=$cadenasellar."|".trim(ReglasXCadena($datosdos[3]));
			$cadenasellar=$cadenasellar."|".trim(ReglasXCadena($myrowtags['municipioexpedido'].','.$myrowtags['estadoexpedido']));
			$cadenasellar=$cadenasellar."|".ReglasXCadena($datosdos[5]);
			//$cadenasellar=$cadenasellar."|".trim(ReglasXCadena($datos[13]));
			...
			"noIdentificacion"=>trim($datos[2]),
			"descripcion"=>trim($datos[4]),
			"valorUnitario"=>trim(FormatNumberERP($datos[5])),
			"importe"=>trim(FormatNumberERP($importe))
			)
			);
			$cadenasellar=$cadenasellar."|".trim(ReglasXCadena($datos[3]))."|".trim(ReglasXCadena($unidades));
			$cadenasellar=$cadenasellar."|".trim(ReglasXCadena($datos[2]))."|".trim(ReglasXCadena($datos[4]));
			//echo $cadenasellar;
			$totalimporte=$totalimporte+$importe;
			$cadenasellar=$cadenasellar."|".trim(ReglasXCadena(FormatNumberERP($datos[5])))."|".trim(ReglasXCadena(FormatNumberERP($importe)));
			...
			cargaAtt($traslado, array("impuesto"=>trim($datos[1]),
			"tasa"=>trim($datos[2]),
			"importe"=>trim(FormatNumberERP($datos[3]))
			)
			);

			$impuestofact=$impuestofact+trim($datos[3]);
			$cadenasellarimp=$cadenasellarimp."|".trim(ReglasXCadena($datos[1]))."|".trim(ReglasXCadena($datos[2]))."|".trim(ReglasXCadena(FormatNumberERP($datos[3])));
		}elseif($cad>=4 and $datos[0]=='6'){
			...
			cargaAtt($retenido, array("impuesto"=>trim($datos[1]),
							"importe"=>trim(FormatNumberERP(abs($datos[3]))))
			);

			$impuestoretenido=$impuestoretenido+trim($datos[3]);
			$cadenasellar=$cadenasellar."|".trim(ReglasXCadena($datos[1]))."|".trim(ReglasXCadena(FormatNumberERP(abs($datos[3]))));
			...
			while ($ind<count($datos)){
				...
				if ($cant < 0 ){
					...
					"Importe"=>FormatNumberERP(abs($cant))
					)
					);
				}
				else{
					...
					"TasadeTraslado"=>$porc,
					"Importe"=>FormatNumberERP($cant)
					)
					);
				}
				$cadimp.="|".$desc."|".$porc."|".FormatNumberERP(abs($cant));
			
			
				$ind+=3;
			}
			cargaAtt($impLocal,array("version"=>"1.0",
			"TotaldeRetenciones"=>FormatNumberERP($tRet),
			"TotaldeTraslados"=>FormatNumberERP($tTras)
			)
			);
			
			$cadimp = utf8_encode("|1.0|".FormatNumberERP($tRet)."|".FormatNumberERP($tTras).trim($cadimp));
		}
	...
	if ($banderaimpuestosretenidos==true){
		$impuestos->SetAttribute("totalImpuestosRetenidos",FormatNumberERP(abs($impuestoretenido)));
	}
	...
	if ($banderaimpuestosretenidos==false){
		...
		$cadenasellar=$cadenasellar."|".ReglasXCadena(FormatNumberERP(abs($impuestoretenido)));//."||";
	}
	
	if ($banderaimpuestos==true){
		$impuestos->SetAttribute("totalImpuestosTrasladados",FormatNumberERP($impuestofact));
	}else{
		$impuestos = $xml->createElement("cfdi:Impuestos");
		$impuestos = $root->appendChild($impuestos);
	}
	...
	if ($banderaimpuestos==false){
		...
		$cadenasellar=$cadenasellar.$cadenasellarimp."|".ReglasXCadena(FormatNumberERP($impuestofact));//."||";
	}

...
Class Cart {
	...
	function update_cart_item( $UpdateLineNumber, $Qty, $Price, $Disc, $Narrative, $UpdateDB='No', $ItemDue, $POLine,$DiscPercent1,$DiscPercent2,$AlmacenStock,$warranty,$servicestatus,$QuantityDispatched,$generakit,$showdescription=1,$modified=0,$clavepresupuestal=''){
	...
	//	echo 'descripcion:'.$QuantityDispatched;
		if($showdescription==''){
			$showdescription=1;
		}
		...
		$this->LineItems[$UpdateLineNumber]->modifiedpriceanddiscount = $modified;
                $this->LineItems[$UpdateLineNumber]->clavepresupuestal = $clavepresupuestal;
		//echo $servicestatus;//
		if ($UpdateDB=='Yes'){
			global $db;
		...
		if ($UpdateDB=='Yes'){
			...
			if ($this->Some_Already_Delivered($LineNumber)==0){
				...
				if($row = DB_fetch_array($result)) {
					...
					// No s� si habilitar esta parte para que tambi�n quite el encabezado de la orden de compra asociada al pedido de venta.
...
if($this->DispatchTaxProvince==''){
	...
	// Valida si impuesto es fiscal
	$sql="Select empresafiscal
		from tags inner join legalbusinessunit on tags.legalid=legalbusinessunit.legalid
		where tagref=".$this->Tagref;
	$ResultFiscal = DB_query($sql,$db,$ErrMsg,$DbgMsg,true);
	if ($RowFiscal = DB_fetch_array($ResultFiscal)) {
		//echo "<br>entra";
		$EmpresaFiscal = $RowFiscal['empresafiscal'];
...
Class LineDetails {
		...
		global $db;
			...
			$this->Quantity = $Qty;
			//echo '<br>cantidad:'.$Qty.'producto:'.$StockItem;
			$this->Price = $Prc;
			...
			//echo '<br>margen:'.$Qty.'f'.$QtyInvoiced.'cant facturada'.$this->QtyDispatched ;
...
global $db;
	...
	if ($_SESSION['LimitxDepto']==1 and $tipodefacturacion==10 and $validarCredito==true){
	...
	}elseif(($_SESSION['CheckCreditLimits']==2 or $_SESSION['CheckCreditLimits']==1)){
		if($CreditAvailable <=0){
			...
			if($invoice==1){
				...
				prnMsg(_('No actualizara informacion de orden por que se ha excedido el limite de credito, limite de : '.$CreditAvailable. ' Cliente: '.$Debtorno),'error');
			}
			return 1;
		}else{

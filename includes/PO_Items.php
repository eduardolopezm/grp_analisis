...
if (isset($_POST['btnGenerarNuevaOrden'])) {
    ...
    //echo "<br>OrderNo: ".$_SESSION['PO'.$identifier]->OrderNo;
    //echo "<br>selectProveedorCambiar: ".$_POST['selectProveedorCambiar'];
}
        ...
        if ($_SESSION['ExistingPurchOrder']==0) {
            ...
            foreach ($_SESSION['PO'.$identifier]->LineItems as $POLine) {
                ...
                if (isset($_POST['AutorizaOrden'])) {
                    ...
                    //echo "<br>GRN: ".$GRN;
                    // Log Presupuesto
                    $descriptionLog = "Autorización de Orden de Compra ".$_SESSION['PO'.$identifier]->OrderNo2;
                    $agregoLog = fnInsertPresupuestoLog($db, $_SESSION['PO'.$identifier]->suficienciaType, $_SESSION['PO'.$identifier]->suficienciaTransno, $tagref, $_POST['clavepresupuestal_'.$POLine->LineNo], $PeriodNo, $totalcompra, 263, "", $descriptionLog); // Suficiencia Automatica
                    $agregoLog = fnInsertPresupuestoLog($db, 555, $GRN, $tagref, $_POST['clavepresupuestal_'.$POLine->LineNo], $PeriodNo, $totalcompra, 258, "", $descriptionLog); // Abono
                    $agregoLog = fnInsertPresupuestoLog($db, 555, $GRN, $tagref, $_POST['clavepresupuestal_'.$POLine->LineNo], $PeriodNo, $totalcompra * -1, 259, "", $descriptionLog); // Cargo
                    //echo "<br>realizo movimientos 1055";
                    
                    // Estatus Autorizado Suficiencia Automatica
                    $sql = "UPDATE tb_suficiencias SET nu_estatus = 4 WHERE sn_orderno = '".$_SESSION['PO'.$identifier]->OrderNo."'";
                    $result =DB_query($sql, $db);
            ...
            $sql = "UPDATE purchorders SET
                    ...
                    orddate = '" . $_SESSION['PO'.$identifier]->Orig_OrderDate . "',
                ...
                //  echo '<pentraaaaaaaaa';
                    ...
                    if ($_SESSION['ExistingPurchOrder'] != 0) {
                        if (isset($_POST['AutorizaOrden'])) {
                            ...
                            $descriptionLog = "Autorización Orden de Compra ".$_SESSION['PO'.$identifier]->OrderNo2;
                            $agregoLog = fnInsertPresupuestoLog($db, $_SESSION['PO'.$identifier]->suficienciaType, $_SESSION['PO'.$identifier]->suficienciaTransno, $tagref, $_POST['clavepresupuestal_'.$POLine->LineNo], $PeriodNo, $totalcompra, 263, "", $descriptionLog); // Suficiencia Automatica
                            $agregoLog = fnInsertPresupuestoLog($db, 555, $GRN, $tagref, $_POST['clavepresupuestal_'.$POLine->LineNo], $PeriodNo, $totalcompra, 258, "", $descriptionLog); // Abono
                            $agregoLog = fnInsertPresupuestoLog($db, 555, $GRN, $tagref, $_POST['clavepresupuestal_'.$POLine->LineNo], $PeriodNo, $totalcompra * -1, 259, "", $descriptionLog); // Cargo
            ...
            if ($GoodProcess==true) {
                //echo '<br><br>';
                prnMsg(_('La Orden de Compra') . ' No. ' . $_SESSION['PO'.$identifier]->OrderNo2 . ' ' . _('se ha actualizado exitosamente.'), 'success');
...
//     echo '</table>';
    ...
    */
    ...
    <div class="col-md-6 col-xs-12">
        <component-label-text label="Requisición:" id="txtOfAuto" name="txtOfAuto" value="<?php echo $_SESSION['PO'.$identifier]->RequisitionNo; ?>" style="font-size: 12pt;"></component-label-text>
        ...
        <?php
        echo '<component-label-text label="Fecha Requerida:" id="txtFechaReq" name="txtFechaReq" value="'.$_SESSION['PO'.$identifier]->Orig_OrderDate.'"></component-label-text>';
...
?>  
    ...
    <div class="col-md-6 col-xs-12">
        <div class="form-inline row">
          ...
          <div class="col-md-9">
              <select id="selectProveedorCambiar" name="selectProveedorCambiar" class="form-control selectProveedorCambiar">
                <?php
                $qry = "SELECT distinct supplierid, CONCAT(supplierid, ' - ' , suppname) as suppname FROM suppliers LIMIT 100";
      ...
      <div class="form-inline row">
        ...
        <?php if ($_SESSION['PO'.$identifier]->separarOrdenCompra == 'P') : ?>
          <component-button type="submit" id="btnGenerarNuevaOrden" name="btnGenerarNuevaOrden" value="Genera Orden de Compra" class="glyphicon glyphicon-plus"></component-button>
        <?php endif ?>
    ...
    if (count($_SESSION['PO'.$identifier]->LineItems)>0 and !isset($_GET['Edit'])) {
        ...
        foreach ($_SESSION['PO'.$identifier]->LineItems as $POLine) {
            ...
            if ($POLine->Deleted==false) {
                ...
                if ($_SESSION['PO'.$identifier]->separarOrdenCompra == 'P') {
                    $checkboxPartida = "<br><input type='checkbox' name='partidasProveedor[]' value='$POLine->PODetailRec' />";
                }
                $recibidas = "";
                ...
                echo "<td align='center'>" . $POLine->LineNo . $checkboxPartida . "</td> ";
        ...
        echo '<tr><td colspan="6" style="text-align: right;">' . _('Totales:') . '</td><td style="text-align: right;">$ ' . $DisplayTotal . '</td>';

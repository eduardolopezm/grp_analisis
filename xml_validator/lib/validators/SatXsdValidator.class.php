...
class SatXsdValidator extends SatValidator {
	...
	public function validateOnComprobanteFactory($xmlObj) { // cambiar a version
		$this->xsdPath = realpath(dirname(__FILE__)) . '/../sat/v'. $this->comprobante->getVersion().'/xsd/cfdv'. $this->comprobante->getVersion().'.xsd';
		$xmlObj->schemaValidate($this->xsdPath);
		$this->parseErrors();
	}

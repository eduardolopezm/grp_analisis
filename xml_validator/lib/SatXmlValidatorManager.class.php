...
class SatXmlValidatorManager {
	...
	public function __construct($path, $db) {
		...
		$this->comprobante = ComprobanteFactory::getComprobante($path, $db, $this);
	}
	...
	public function validate() {
		if ($this->comprobante != null) {
			...
			$error->setNode('XML');
			$error->setAttribute('Comprobante');
			$error->setValue('CFDI');
	...
	public function getValidatorByClassName($className) {
		if( isset($this->validators[$className]) and !empty($this->validators[$className])) {
	...
	public function addExternError($code, $msg, $node, $attribute, $value) {
		$error = new SatXmlError();
		$error->setCode($code);

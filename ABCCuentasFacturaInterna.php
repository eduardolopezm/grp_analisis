...
$men=$_GET['menu'];
	...
	if ($listado == "1"){
		...
		$sql="select * from chartinternalinvoice";
		$res=(db_query($sql,$db));
		}
	...
	if ($men == "2" ){
		...
		$sql="select * from chartinternalinvoice where accountcode='" . $_GET['cuenta'] . "'";
		$res=(db_query($sql,$db));
	...
	if ($ok_process == true && $_POST['boton'] == 'Procesar' ){
			...
			$sql="select count(*) as cuenta from chartinternalinvoice where accountcode='". $_POST['cuenta1'] ."'";
			$res=(db_query($sql,$db));
			$myrow=db_fetch_row($res);
		
			if ($myrow[0]>0){
				echo " <b> <font color=#FF0000> Error ya existe la cuenta: </font> </b><b>" .$_POST['cuenta1']."</b>";
				echo "<center <a href='ABCCuentasFacturaInterna.php?men=1' > REGRESAR </a> </center>";

				}
			else {
				$sql="insert into chartinternalinvoice(accountcode,concept)";
			...
			$sql="select count(*) as cuenta from chartinternalinvoice where accountcode like '%". $_POST['cuenta1'] ."%'";
			$res=(db_query($sql,$db));
			$myrow=db_fetch_row($res);
			if ($myrow[0]<=0){
				...
				$sql="select * from chartinternalinvoice where accountcode like '%".$_POST['cuenta1']."%'";
				$res=(db_query($sql,$db));
				$listado="2";
				echo "<br>";
				echo "<br>";
			...
			$sql="UPDATE chartinternalinvoice SET concept='$_POST[concepto]' WHERE accountcode='$myrow[accountcode]'";
			$res=(db_query($sql,$db));
			$listado="1";
			$men="1";
			$sql="select * from chartinternalinvoice";
			...
			echo"<META HTTP-EQUIV='refresh' CONTENT='2; URL=ABCCuentasFacturaInterna.php?men=1'>";
			
	}elseif($_POST['botsi'] == 'ELIMINAR'){
			$sql="DELETE FROM chartinternalinvoice WHERE accountcode='$myrow[accountcode]'";
			$res=(db_query($sql,$db));
			$listado="1";
			$men="1";
			$sql="select * from chartinternalinvoice";
...
if ($men != "2" && $men != "3"){
	...
	echo "<br>";
		...
		echo "<H2> <center> ABC CUENTAS FACTURAS INTERNAS </center> </H2>";
	...
	echo "<tr>";
		...
		echo "</td>";
		echo "<td>";
			echo "<a href='ABCCuentasFacturaInterna.php?menu=2&cuenta=" . $myrow['accountcode'] . "'> Editar </a>";
		echo "</td>";
		
		echo "<td>";
			echo "<a href='ABCCuentasFacturaInterna.php?menu=3&cuenta=" . $myrow['accountcode'] . "'> Eliminar </a>";
		echo "</td>";

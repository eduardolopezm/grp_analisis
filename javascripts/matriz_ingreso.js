// objetos de configuración
	...
	window.buscadoresConfiguracion.stockact.cuentasAprobadas = ['1.1.1'];
...
$(document).ready(function() {
	...
	$('#guardar').on('click',function(){
		var	params = getParams(idForma), campos = getMatchets(idForma), msg = '',
			nombreCampo = {
				...
				'txtPartidaEspecifica': 'Partida',
				'selectCapitulo': 'Capitulo'
			},
		...
		$.each(campos,function(index, el) {
			...
			if(el=="selectUnidadEjecutora"&& ($("#"+el).val()=="00" || $("#"+el).val()=="-1")){ return; } // Línea adicional para este módulo
			if(!params.hasOwnProperty(el)){ return; }
			// if(esepcion.indexOf(el)!==-1){ return; }
			if(params[el]!=0&&params[el]!=-1){ return; }

			if((el=="txtpp" && $("#"+el).val() !="-1") || (el=="selectCapitulo" && $("#"+el).val() !="-1") ){
				return;
			}

			if(el=="txtpp" && $("#"+el).val() =="-1"){
				if($("#selectCapitulo").val() !="-1"){
					return;
				}
			}

			if(el=="selectCapitulo" && $("#"+el).val() =="-1"){
				if($("#txtpp").val() !="-1"){
					return;
				}
			}
	...
	$(document).on('cellselect','#tablaGrid',function(e){
		var	ln_clave = "", ln_capitulo="", CamposABloquear = false; // Línea adicional para este módulo
			...
			.then(function(res){
				...
				if(res.success){
					...
					if(ln_clave){
						...
						if(ln_capitulo > 0){
							$('#selectCapitulo').multiselect('select', ln_capitulo);
						}else{
...
function inicioPanel() {
	...
	this.rootFu = 0;
	this.url = getUrl();
	this.modelo = this.url+'/modelo/matriz_ingreso_Modelo.php';
...
function llenaTabla(data) {
	...
	var	data = data||[], el = 'contenedorTabla', tabla = 'tablaGrid', nameExcel = 'Matriz de Ingresos'
...
/**
 * Función para obtener la descripción de la Partida especifica
 * @param {[type]} elemento Recibe la caja de texto
 * @return {[type]}		[description]
 */
function fnTraeInformacionPartidaEspecifica(valor) {
	// Obtener la descripcion de la partida generica
	// Traer descripción de la partida generica
	var	descripcionPartida = fnInformacionPartidaGenericaGeneralMatrices(valor.substring(0,3));
	$("#categoryid").val(valor.substring(0,3));
	$("#categorydescription").val(""+descripcionPartida);

...
//		str += '<img id="i' + this.obj + nodeId + '" src="' + ((node._io) ? node.iconOpen : node.icon) + '" alt="" />';
		str += '<img id="i' + this.obj + nodeId + '" src="' + ((node._io) ? node.iconOpen : node.icon) + '" alt="" ';
		if (node.target){
			str+='style="cursor:hand" title="Presiona para expandir el arbol desde aqui" onclick="'+node.target+'" />';
		}else{
			str+='/>'
		}
	...
	if (node.url) {
		...
		str += '<a id="s' + this.obj + nodeId + '" class="' + ((this.config.useSelection) ? ((node._is ? 'nodeSel' : 'node')) : 'node') + '" target="_self" href="' + node.url + '"';

...
function fnObtenerInformacion() {
	var jerarquia = "";
	var selectJerarquia = document.getElementById('jerarquia');
    for ( var i = 0; i < selectJerarquia.selectedOptions.length; i++) {
        //console.log( unidadesnegocio.selectedOptions[i].value);
        if (i == 0) {
            jerarquia = "'"+selectJerarquia.selectedOptions[i].value+"'";
        }else{
            jerarquia = jerarquia+", '"+selectJerarquia.selectedOptions[i].value+"'";
        }
    }

	var tipoSol = "";
	var selectTipoSol = document.getElementById('tipoSol');
    for ( var i = 0; i < selectTipoSol.selectedOptions.length; i++) {
        if (i == 0) {
            tipoSol = "'"+selectTipoSol.selectedOptions[i].value+"'";
        }else{
            tipoSol = tipoSol+", '"+selectTipoSol.selectedOptions[i].value+"'";
        }
    }

    var tipoGasto = "";
	var selectTipoGasto = document.getElementById('tipoGasto');
    for ( var i = 0; i < selectTipoGasto.selectedOptions.length; i++) {
        if (i == 0) {
            tipoGasto = "'"+selectTipoGasto.selectedOptions[i].value+"'";
        }else{
            tipoGasto = tipoGasto+", '"+selectTipoGasto.selectedOptions[i].value+"'";
        }
    }

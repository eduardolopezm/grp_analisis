...
$(document).ready(function() {
    ...
    $('#nuevo').on('click',function(){
        $('#tituloModal').html('Nueva Relación PP - Partida');
        $('#modalRelacionPpPartida').modal('show');
    });
    // comportamiento del boton de guardado
    $('#guardar').on('click',function(){
        var params = getParams(idForma), campos=getMatchets(idForma), msg=''
            ,nombreCampo={'pp':'PP','partida':'Partida'}
    ...
    $(document).on('cellselect','#tablaGrid',function(e){
        ...
        .then(function(res){
            ...
            if(res.success){
                ...
                $('#modalRelacionPpPartida #tituloModal').html("Edición Relación PP - Partida");
                $('#modalRelacionPpPartida').modal('show');
            }
        });
    })
...
function inicioPanel() {
    ...
    this.rootFu = 0;
	this.url = getUrl();
	this.modelo = this.url+'/modelo/abcRelacionPpPartidaModelo.php';
    ...
    $('#modalRelacionPpPartida').on('hidden.bs.modal',function(){
        var limpiarSelects = ['pp','partida'];
        $.each(limpiarSelects,function(index,el){ $('#'+el).multiselect('select',0).multiselect('rebuild'); });
        $('#modalRelacionPpPartida').find('#identificador').remove();
    });
...
function llenaTabla(data) {
	...
	var data = data||[], el = 'contenedorTabla', tabla = 'tablaGrid', nameExcel = 'Relación PP - Partida'
        , tblObj, tblTitulo, tblExcel=[0,1], tblVisual=[0,1,2]; //,3
    tblObj = [
        { name: 'pp', type: 'string'},// 0
        { name: 'partida', type: 'string'},// 1
        { name: 'descripcion', type: 'string'},// 2
        //{ name: 'modificar', type: 'string'},// 3
        { name: 'eliminar', type: 'string'},// 4
        { name: 'identificador', type: 'string'},// 5
    ];
    tblTitulo = [
        { text: 'PP', datafield: 'pp', editable: false, width: '47%', cellsalign: 'center', align: 'center' },// 0
        { text: 'Partida', datafield: 'partida', editable: false, width: '46%', cellsalign: 'center', align: 'center' },// 1
        //{ text: 'Modificar', datafield: 'modificar', editable: false, width: '25%', cellsalign: 'center', align: 'center' },// 2
        { text: 'Eliminar', datafield: 'eliminar', editable: false, width: '7%', cellsalign: 'center', align: 'center' },// 3 antes todas a width: '25%'
    ];

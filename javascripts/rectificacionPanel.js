...
$( document ).ready(function() {
    ...
    if (document.querySelector(".selectEstatusCompromiso")) {
		...
		fnSelectGeneralDatosAjax('.selectEstatusCompromiso', dataObj, 'modelo/rectificacionPanelModelo.php', 0);
	}
...
function fnObtenerRegistrosSuficiencia() {
	...
	$.ajax({
	    method: "POST",
	    dataType:"json",
	    url: "modelo/rectificacionPanelModelo.php",
	    data:dataObj
	})
	.done(function( data ) {
		//console.log("Bien");
		if(data.result){
			...
			var columnasExcel= [1, 2, 3, 4, 5, 7, 8, 10, 12];
			var columnasVisuales= [0, 1, 2, 3, 4, 6, 7, 8, 10, 12, 14];

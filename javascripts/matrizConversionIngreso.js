...
window.buscadores = [ "stockact", "accountegreso","stockactBuscador", "accountegresoBuscador" ];
...
window.buscadoresConfiguracion.stockact.cuentasAprobadas = ['1','2'];
window.buscadoresConfiguracion.accountegreso.cuentasAprobadas = ['1','4'];


//Busqueda Filtro

window.buscadoresConfiguracion.stockactBuscador.origenDatos = "cuentasTotales";
window.buscadoresConfiguracion.accountegresoBuscador.origenDatos = "cuentasTotales";
window.buscadoresConfiguracion.stockactBuscador.cuentasAprobadas = ['1','2'];
window.buscadoresConfiguracion.accountegresoBuscador.cuentasAprobadas = ['1','4'];
...
$('#guardar').on('click',function(){
    if(isCri){
        ...
        $('#criValidator').removeClass('hidden');
    }
});
...
$("#busquedaConcepto").keyup(function(event){
    DescripcionPartida = ( this.value.length > 3 ? fnInformacionPartidaGenericaGeneralMatrices(this.value) : "Sin Conversión por Rubro Ingreso seleccionado" );
    $( "#DescripcionPartida" ).html( DescripcionPartida ? DescripcionPartida : "El Clasificador por Rubro Ingreso no existe" );
});
...
this.rootFu = 0;
this.url = getUrl();
this.modelo = this.url+'/modelo/matrizConversionIngresoModelo.php';
...
console.log('listo el panel matriz por Rubro Ingreso');
...
var	data = data||[], el = 'contenedorTabla', tabla = 'tablaGrid', nameExcel = 'Matriz Conversión de Clasificador por Rubro Ingreso'
...
tblTitulo = [
    { text: 'Clasificador Rubro Ingreso', datafield: 'categoryid', editable: false, width: '11%', cellsalign: 'center', align: 'center' },// 1
    { text: 'Descripción', datafield: 'categorydescription', editable: false, width: '25%', cellsalign: 'center', align: 'center' },// 2
    { text: 'Cargo', datafield: 'stockact', editable: false, width: '3%', cellsalign: 'center', align: 'center' },// 4
    { text: 'Cuenta Cargo', datafield: 'nombreCargo', editable: false, width: '19%', cellsalign: 'center', align: 'center' },// 5
    { text: 'Abono', datafield: 'accountegreso', editable: false, width: '3%', cellsalign: 'center', align: 'center' },// 6
    ...
    { text: 'Eliminar', datafield: 'eliminar', editable: false, width: '10%', cellsalign: 'center', align: 'center' },// 10
];
...
var	CamposABloquear = [{
        ...
        'categoryid':'Clasificador por Rubro Ingreso',
...
/**
* Función para obtener la descripción de la Clasificador Rubro Ingreso
...
// Obtener la descripcion de la partida generica
    ...
    if (value.length > 3) {
        ...
        var	descripcionPartida = fnInformacionPartidaGenericaGeneralMatrices(value);
        $("#categorydescription").val(""+descripcionPartida);
        isCri = true;
        
        if(!descripcionPartida){
            $("#categorydescription").val("Sin Clasificador por Rubro Ingreso seleccionado");
            isCri = false;
        }
...
/**
* Función para obtener la descripción de la Clasificador Rubro Ingreso
* @param	{[type]}	partidaGenerica Clasificador Rubro Ingreso a 3
* @return	{[type]}	Descripción de la Clasificador Rubro Ingreso
*/
function fnInformacionPartidaGenericaGeneralMatrices(claveCri) {
// Obtiene la descripcion de la partida generica
var	descripcionCri = '';
dataObj = { 
    option: 'muestraDesCRI',
    claveCri: claveCri
};
$.ajax({
    async:false,

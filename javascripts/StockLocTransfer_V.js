...
var url = "modelo/StockLocTransferModelo_V.php";
    ...
    setTimeout(function(){
        ...
        if (document.querySelector(".almacen")) {
            dataObj = { 
              ...
              fnSelectGeneralDatosAjax('.almacen', dataObj, 'modelo/StockLocTransferModelo_V.php', 0);
        }
        if (document.querySelector(".selectTipoProducto")) {
            dataObj = { 
                option: 'mostrarTipoProducto'
            };
            fnSelectGeneralDatosAjax('.selectTipoProducto', dataObj, 'modelo/StockLocTransferModelo_V.php', 0);
        }
 ...
 */
function fnNuevaRequisicion(){
    
    orden ++
    console.log("nuevo elemento"+orden);
    fnAgregarElemento(orden,cont);
...
function fnLoadRequisicion(reference){
     muestraCargandoGeneral();
    var contar = 0;
    dataObj={
        option: 'loadSalida',
    ...
    $.ajax({
        ...
        if (data.result) {
            ...
            var tipo     = "";
            var partida  = "";
            fnObtenerStatus(reference);

            idTransfer = info[0].idTransfer;
            stockid = info[0].stockid;
            shipqty = info[0].shipqty;
            shiploc = info[0].shiploc;
            recloc  = info[0].recloc;
            
            if (stockid.substr(0,1) == 2){
                tipo ="B"
            }else{
                tipo = "I"
            }
            ...
            $("#almacenRem").val(shiploc);
            $('#almacenRem').multiselect('disable');
           
            $('#almacenDes').val(recloc);
            
            $("#almacenDes").multiselect('rebuild');
            $("#selectTipoProducto").val(tipo);
            ...
            for (f in info){
                ...
                idTransfer = info[f].idtransfer;
                stockid = info[f].stockid;
                shipqty = info[f].shipqty;
                comments = info[f].comments;
                partida = info[f].partida;

                
                contar++; 
                fnCargarArticulo(idTransfer,stockid,shipqty,shiploc,recloc,comments,contar,partida); 

            }
            $("#almacenRem").multiselect('disable'); 
            $('#almacenDes').multiselect('disable');  
    ...
    $.ajax({
        ...
        if (data.result) {
            ...
            for (var info in dataJson) {
                ...
                if(tipo == 'B'){
                    fnArticulo(idRequisicion, orden, fEntrega, indiceElement, idPartida, idItem, descItem, unidad,cantidad,precio,total,existencia,clavepresupuestal,descLarga,renglon, viewAnexo, idItemDetalle);
                }else{
...
function fnAgregarElemento(orden,cont){
    ...
    if(ord > 0){
        ...
        cont = 1;
        console.log("hoa",cont)
        indiceIncremento = 1;
    }
    console.log("indiceIncremento",indiceIncremento)
                fnArticulo(orden,indiceIncremento);
...
function fnRemoverElemento(orden){
...
function fnEliminarElemento(orden){
    console.log(orden);
    
    if ( reference >0){
        idTransfer =  $("#idTransfer"+orden).val(); 
        if (idTransfer != '' ){
            dataObj = {
                option: 'eliminarElementos',
            ...
            $.ajax({
                ...
                if (data.result) {
                    ...
                    //fnReindexar(); 
                    //orden = orden - 1;   
                    fnRemoverElemento(orden);
                }else{
                    //muestraMensaje('No se elimino el elemento a la Requisición: '+ noReq, 3, 'divMensajeOperacion', 5000);  
                    muestraModalGeneralConfirmacion(3,'<h3><p><i class="glyphicon glyphicon-info-sign text-danger" aria-hidden="true"></i>Error</p></h3>',
                    'Hubo un error al ejecutar la consulta no se pudo eliminar el elemento de la salida de almacén '+idReq+'.¿Desea volver a intentarlo?',"",
...
function fnClaveArticulo(orden) {
    ...
    $("#idMainListContentArticulo select[id*=selectCvePartidaEspecifica]").each(function(index, el) {
        ...
        var clavegrid  = $("#selectCveArticulo" + numero).val();
        console.log("clvegrd",clavegrid)
        if (clavegrid ==  cveprod){
            contador ++; 
        }
    ...
    if ( contador > 1) {
        ...
        ocultaCargandoGeneral(); 
        muestraModalGeneral(3,'Error de Datos', 'Ya tiene agregada la Clave de articulo '+cveprod);
        fnCargaPartidaCvePpto(numero);
        return; 
    }
...
function fnCargarArticulo(idTransfer,stockid,shipqty,shiploc,recloc,comments,numero, partida){
    ...
    $(containerArticulo).append('<div class="row p0 m0">'+
            '<ol id=idElementArticulo' + numero + ' class="idElementArticulo col-lg-12 col-md-12 col-sm-12 p0 m0">' +
                '<input type="hidden" id="idTransfer'+numero +'"  name="idTransfer'+numero +'" value="'+idTransfer+'" class ="idTransfer">'+
                '<li id="idEliminarArticulo' + numero + '" class="w5p pt5"><span class="btRemoveArticulo btn btn-danger btn-xs glyphicon glyphicon-remove" id="btRemoveArticulo' + numero + '" title="Eliminar" onclick="fnEliminarElemento('+numero+', \'B\');"></span></li>' + 
                '<li id="idNumArticulo' + numero + '" class="w5p pt5"><input type="text" id="nProd' + numero + '" class="hide" value="'+numero+'"/><label  class="hide indice w100p text-center" type="text" id="numArticulo' + numero + '">'+numero+'</label><label class="num w100p text-center" type="text" id="num' + numero + '">'+numero+'</label></li>' + 
                '<li id="idCvePartida'+numero+'" class="w10p p0"><select id="selectCvePartidaEspecifica' + numero + '" name="selectCvePartidaEspecifica' + numero + '" class="w100p form-control selectCvePartidaEspecifica" onchange="fnCargaPartidaCvePpto('+numero+');"></select></li>'+
                '<li id="idCvePartidaCveArticulo' + numero + '" class="w10p p0"><select id="selectCveArticulo' + numero + '" name="selectCveArticulo' + numero + '" class="selectCveArticulo form-control" onchange="fnClaveArticulo('+numero+');"><option value="0">Cve ...</option></select></li>'+
                '<li id="idCvePartidaDescArticulo' + numero + '" class="w20p p0"><select id="selectArticulo' + numero + '" name="selectArticulo' + numero + '" class="form-control selectArticulo" onchange="fnDescArticulo('+numero+');"><option value="0">Articulos ...</option></select></li>'+
                '<li id="idUMArticulo' + numero + '" class="w10p pt5"><label  class="w100p addedUMArticulo" type="text" id="addedUMArticulo' + numero + '"></label></li>'+
                '<li id="idCantidadArticulo' + numero + '" class="w13p pt5"><input onkeypress="return soloNumeros(event)" class="addedCantidadArticulo vacia num w100p text-center" type="text" id="addedCantidadArticulo' + numero + '" placeholder="Cantidad" onblur="fnCalculaTotal('+numero+');" readonly="readonly" disabled="disabled"></li>'+
                '<li id="idRenglonAnexoArticulo' + numero + '" class="w25p pt5"><input class="addObservaciones empty w100p mr2" type="text" id="addObservaciones' + numero + '" value="'+comments+'" placeholder="Observaciones"/>'+
                '</ol>' +
        '</div>');

    $('#idMainListContentArticulo').append(containerArticulo);
    fnEjecutarVueGeneral('precioart' + numero );

    // Cambios pruebas, variable para deshabilitar pagina
    var deshabilitaPagina = 0;

   
    if(status == 'Recibido' || status == 'Por Entregar'){
        bloquear = true
        $("#idBtnAgregarArticulo").addClass('hide');
        $("#idBtnAgregarServicio").addClass('hide');
        $("#btRemoveArticulo" + numero).addClass('hide');
        $("#btRemoveArticulo" + numero).attr('disabled', 'disabled');
        ...
        $("#addedCantidadArticulo" + numero).prop("readonly", false); 
        $("#addedCantidadArticulo" + numero).attr('disabled', false);
        // Cambios pruebas, Se deshabilita caja de precio para ser editada
        $("#precioart" + numero).prop("readonly", false); 
        $("#precioart" + numero).attr('disabled', false);
        $("#addedPEArticulo" + numero).prop("readonly", false);
        $("#addedPEArticulo" + numero).attr('disabled', false);
    }
    fnCargarPartidaProducto('', '', numero); 
    fnFormatoSelectGeneral("#selectCvePartidaEspecifica"+numero);

    $("#selectCvePartidaEspecifica" + numero).val(partida);
    $("#selectCvePartidaEspecifica"+numero).multiselect('rebuild');
    fnCargaPartidaCvePpto(numero);

    setTimeout(function () {
       // muestraCargandoGeneral(); 
        $("#selectCveArticulo" + numero).val(stockid);
        $("#selectCveArticulo"+numero).multiselect('rebuild'); 
        fnClaveArticulo(numero);
       
        $("#addedCantidadArticulo" + numero).val(shipqty);
        $("#addObservaciones" + numero).val(comments);

        if ( bloquear){
            setTimeout(function () {
                muestraCargandoGeneral(); 
                $(".selectArticulo").multiselect('disable'); ocultaCargandoGeneral();
            }, 2200);
            $("#selectCveArticulo"+numero).multiselect('disable');
            $("#selectCvePartidaEspecifica"+numero).multiselect('disable');
            $("#addObservaciones"+numero).attr("disabled","disabled")
            $("#addedCantidadArticulo"+numero).attr("disabled","disabled")
            ocultaCargandoGeneral();
        }
     //   ocultaCargandoGeneral();
    }, 2100);
   ...
   /* fnCargaPartidaCvePpto(numero);
    $("#selectCvePartidaEspecifica" + orden).val(stockid);
    $("#selectCvePartidaEspecifica"+orden).multiselect('rebuild');*/

}


function fnArticulo(orden, indiceIncremento ){
   
    var contenidoClaves= "";
    var perfilid = $("#idperfilusr").val();
    console.log("aqui llega", indiceIncremento)

    $(containerArticulo).append('<div class="row p0 m0">'+
            '<ol id=idElementArticulo' + orden + ' class="idElementArticulo col-lg-12 col-md-12 col-sm-12 p0 m0">' +
                '<input type="hidden" id="idTransfer'+orden +'"  name="idTransfer'+orden +'" value="" class ="idTransfer">'+
                '<li id="idEliminarArticulo' + orden + '" class="w5p pt5"><span class="btRemoveArticulo btn btn-danger btn-xs glyphicon glyphicon-remove" id="btRemoveArticulo' + orden + '" title="Eliminar" onclick="fnEliminarElemento('+orden+', \'B\');"></span></li>' + 
                '<li id="idNumArticulo' + orden + '" class="w5p pt5"><input type="text" id="nProd' + orden + '" class="hide" value="'+orden+'"/><label  class="hide indice w100p text-center" type="text" id="numArticulo' + orden + '">'+ orden+'</label><label class="num w100p text-center" type="text" id="num' + orden + '">'+indiceIncremento+'</label></li>' + 
                ...
                '<li id="idCvePartidaDescArticulo' + orden + '" class="w20p p0"><select id="selectArticulo' + orden + '" name="selectArticulo' + orden + '" class="form-control selectArticulo" onchange="fnDescArticulo('+orden+');"><option value="0">Articulos ...</option></select></li>'+
                '<li id="idUMArticulo' + orden + '" class="w10p pt5"><label  class="w100p addedUMArticulo" type="text" id="addedUMArticulo' + orden + '"></label></li>'+
                '<li id="idCantidadArticulos' + orden + '" class="w13p pt5"><input onkeypress="return soloNumeros(event)" class="addedCantidadArticulo vacia num w100p text-center" type="text" id="addedCantidadArticulo' + orden + '" placeholder="Cantidad" onblur="fnCalculaTotal('+orden+');" readonly="readonly" disabled="disabled"></li>'+
                 '<li id="idRenglonAnexoArticulo' + orden + '" class="w25p pt5"><input class="addObservaciones empty w100p mr2" type="text" id="addObservaciones' + orden + '" value="" placeholder="Observaciones"/>'+
            '</ol>' +
        '</div>');
    ...
    fnCargarPartidaProducto('', '', orden); 
    fnFormatoSelectGeneral("#selectCvePartidaEspecifica"+orden);
}

// modfica los valores de cada renglon de articulos
function fnModificarArticulo(reference,numero){
    console.log("modificando orden: %s",reference);
    console.log("modificando req: %s",numero);
    var claveArticulo  = $("#selectCveArticulo" + numero).val();
    var descArticulo   = $("#selectArticulo"+numero).val();
    var addedCantidadArticulo = $("#addedCantidadArticulo" + numero).val();
    var addObservaciones  = $("#addObservaciones" + numero).val();
    var idTransfer = $("#idTransfer" + numero).val();
    var validacion = false; 
    console.log("idtransfer",idTransfer)
    if (idTransfer > 0){
        dataObj = {
            option: 'modificarElementosSalida',
            claveArticulo: claveArticulo,
            descArticulo: descArticulo,
            addedCantidadArticulo: addedCantidadArticulo,
        ...
        $.ajax({
            ...
            if (data.result) {
                ...
                muestraMensaje('No se Modifico la Salida de almacén', 3, 'divMensajeOperacion', 5000);
                return; 
            }
        }).fail(function(result) {
            console.log("ERROR");
        ...
        folio = reference ; 
        fnValidarExistencia(numero);
        if ( existencia == null)
            existencia = -1; 
        console.log ("la existencia en modificacion ",existencia)
        if (existencia > 0){
        
            fnGuardarTransferenciaNueva(numero);
            validacion = true; 
        }else{
            let articulo = $("#selectArticulo"+numero+" option:selected").text();
            muestraMensaje("La cantidad solicitada del producto "+articulo+ " sobrepasa a la cantidad que puede transferir", 3, 'divMensajeOperacion', 5000);
        }
    }
    return validacion ; 
}
...
function fnModificarInstrumental(req,orden){
    ...
    var renglon = $("#addObservaciones" + orden).val();
...
function fnValida(){
    ...
    let almacenRem  = $("#almacenRem").val();
    let almacenDes  = $("#almacenDes").val(); 
    detalle = 0; 
    $('#ModalGeneral_Mensaje').addClass('maxh400');
    $('#ModalGeneral_Mensaje').addClass('overflowY');
    
    if ( almacenRem == almacenDes){
        mensaje+= '<p><i class="glyphicon glyphicon-remove-sign text-danger" aria-hidden="true"></i>&nbsp;&nbsp;El almacén de origen no puede ser igual al de destino.</p>';
    ...
    $("#idMainListContentArticulo select[id*=selectCvePartidaEspecifica]").each(function(index, el){
        ...
        obj.cantidadArticulos = $('#addedCantidadArticulo'+numero).val();
        obj.cantidadArticulos = $('#addObservaciones'+numero).val();
...
// inicia el proceso del guardado de la transferencia, dependiendo si es nueva o ya existente
function fnGuardarRequisicion(){
    var modificacionArticulo = false; 
    var agregarArticulo      = false; 
            ...
            if (detalle > 0){
                $("#idMainListContentArticulo select[id*=selectCvePartidaEspecifica]").each(function(index, el) {
                    ...
                    if (existencia > 0){
                        ...
                        modificacionArticulo =  fnModificarArticulo(reference, numero);
                        mensaje              = true;
                ...
                muestraMensaje('Favor de registrar los productos a transferir', 3, 'divMensajeOperacion', 5000);
            }
            if ( mensaje){
                muestraMensaje('Se modifico la salida de almacén con éxito', 3, 'divMensajeOperacion', 5000);
            ...
            if (detalle > 0){
                $("#idMainListContentArticulo select[id*=selectCvePartidaEspecifica]").each(function(index, el) {
                    ...
                    if (existencia > 0 ){
                        ...
                        agregarArticulo = fnGuardarTransferenciaNueva(numero);
                    }else{
                        let articulo = $("#selectArticulo"+numero+" option:selected").text();
                        muestraMensaje("La cantidad solicitada del producto "+articulo+ " sobrepasa a la cantidad que puede transferir", 3, 'divMensajeOperacion', 5000);
                    }
                ...
                if (agregarArticulo){
                    ...
                    muestraModalGeneral(3, titulo, '<p><i class="glyphicon glyphicon-plus-sing text-success" aria-hidden="true"></i> Se guardó exitosamente la transferencia con el folio '+folio+'</p>');
                    reference = folio; 
                    ocultaCargandoGeneral();
                }
...
function fnValidarExistencia(numero){
    let cveArticulo       = $("#selectCveArticulo"+numero).val(); 
    let cantidadArticulo  = $("#addedCantidadArticulo"+numero).val();
    let almacenRem        = $("#almacenRem").val();
    existencia            = 0;
    dataObj = {
        option: 'validarExistencia',
        cveArticulo:cveArticulo,
        cantidadArticulo:cantidadArticulo, 
        almacenRem:almacenRem,
    };
    $.ajax({
        async:false,
        cache:false,
        ...
        if (data.result) {
            arrayData = data.contenido.datos;
            console.log("arrayData",arrayData)
            for (var info in arrayData) {
                valor = arrayData[info].existencia;
            }
...
function fnGuardarTransferenciaNueva(numero){
    let almacenRem        = $("#almacenRem").val();
    let almacenDes        = $("#almacenDes").val(); 
    var partidaEspecifica = $("#selectCvePartidaEspecifica" + numero).val();
    var cveArticulo       = $("#selectCveArticulo"+numero).val(); 
    var descArticulo      = $("#selectArticulo"+numero).val(); 
    let cantidadArticulo  = $("#addedCantidadArticulo"+numero).val();
    let addObservaciones  = $("#addObservaciones"+numero).val(); 
    var result            = false; 
    console.log("guardo"); 
    dataObj = {
            option: 'guardarTransferencia',
            almacenRem: almacenRem,
            status: 'Capturado',
        ...
        $.ajax({
            ...
            muestraMensaje('La trasnferencia no se guardo con exito', 3, 'divMensajeOperacion', 5000);
            ocultaCargandoGeneral();
            console.log("ERROR");
            console.log(result);
            return; 
...
// cancela todo movimiento no guardado en la transferencia
function fnCancelarRequisicion(){
  
    muestraMensaje('Se cancelo todo movimiento no guardado en la transferencia ', 1, 'divMensajeOperacion', 5000); 
...
function fnRegresarPanelRequisicion(){
    location.replace("./StockLocTransfer.php");
}
...
function fnMuestraAnexo(idRequi, ur, ue, ordenElementoRequi, folioAnexo, mbflag, forceShow){
    anexoAsignado = 1;
    var contAnexo = $("#addObservaciones"+ordenElementoRequi).val();
...
function fnAgregaRenglon(idRequi, idAnexo, idAnexoElemento, ordenElementoRequi, ordenAnexo){
    var contAnexoArt = $("#addObservaciones"+ordenElementoRequi).val();
    ...
    if($('input:checkbox[name=selElementoAnexo'+idAnexoElemento+']').is(':checked')) {
        ...
        if (contAnexo == ''){
            ...
            contAnexo = $("#addObservaciones"+ordenElementoRequi).val();
            if(typeof contAnexo === 'undefined'){
                contAnexo = $("#addedReglonServicio"+ordenElementoRequi).val();
...
$(document).on('click','.cerrarModalCancelarReuisicion',function(){
    var encUrl = $('#urlEncriptadaRequisicionInput').val();
    location.replace("./StockLocTransfer.php?"+encUrl);
    //location.replace("./PO_SelectOSPurchOrder.php?");
});

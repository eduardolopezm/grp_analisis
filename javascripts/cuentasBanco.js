...
$(document).ready(function() {
	...
	$('#nuevo').on('click',function(){
		...
		$('#tituloModal').html('<i class="glyphicon glyphicon-info-sign text-success" aria-hidden="true"></i> Nuevo Cuentas Bancarias');
	...
	$('#guardar').on('click',function(){
			var params = getParams(idForma), campos = getMatchets(idForma), msg = '',
				nombreCampo = {
					'currCode':'Moneda',
					'nameBank':'Nombre Cuenta Banco',
					'default':'Default para Facturas',
					'glAccount': 'Codigo GL Cuenta Banco',
					'selectBank': 'Banco'
				},
			...
			$.each(campos,function(index, el) {
				...
				if(el == 'selectUnidadNegocio[]'){ return; }
	...
	$(document).on('cellselect','#tablaGrid',function(e){
		...
		.then(function(res){
			...
			if(res.success){
				...
				$.each(res.tagrefs, function(index, val) {
					$("#selectUnidadNegocio").find("option[value="+val+"]").prop("selected", "selected");
				...
				AlinearSelectsDerecha();
				$('#modalUsoGeneral #tituloModal').html('<i class="glyphicon glyphicon-info-sign text-success" aria-hidden="true"></i> Edición Cuentas Bancarias');
				$('#modalUsoGeneral').modal('show');
				llenaTabla();
...
function inicioPanel() {
	...
	this.rootFu = 0;
	this.url = getUrl();
	this.modelo = this.url+'/modelo/cuentasBancoModelo.php';
...
function llenaTabla(data) {
	...
	tblObj = [
		{ name: 'glAccount', type: 'string'},// 0
		{ name: 'claveBank', type: 'string'},// 0
		{ name: 'nameBank', type: 'string'},// 1
		{ name: 'numBank', type: 'string'},// 4
		{ name: 'addressBank', type: 'string'},// 5
		{ name: 'currCode', type: 'string'},// 5
        { name: 'invoice', type: 'string'},// 5
		{ name: 'bank', type: 'string'},// 5
		{ name: 'modificar', type: 'string'},// 7
		{ name: 'eliminar', type: 'string'},// 8
		{ name: 'identificador', type: 'string'},// 9
	];
	tblTitulo = [
		{ text: 'GL Codigo Cuenta', datafield: 'glAccount', editable: false, width: '12%', cellsalign: 'center', align: 'center' },// 1
		{ text: 'Nombre Cuenta Banco', datafield: 'bank', editable: false, width: '12%', cellsalign: 'center', align: 'center' },// 0
		{ text: 'Codigo Cuenta Banco', datafield: 'claveBank', editable: false, width: '12%', cellsalign: 'center', align: 'center' },// 3
		{ text: 'Numero Cuenta Banco', datafield: 'numBank', editable: false, width: '12%', cellsalign: 'center', align: 'center' },// 4
		{ text: 'Direccion Banco', datafield: 'addressBank', editable: false, width: '10%', cellsalign: 'center', align: 'center' },// 5
		{ text: 'Moneda', datafield: 'currCode', editable: false, width: '8%', cellsalign: 'center', align: 'center' },// 5
		{ text: 'Default para Facturas', datafield: 'invoice', editable: false, width: '10%', cellsalign: 'center', align: 'center' },// 3
		{ text: 'Banco', datafield: 'nameBank', editable: false, width: '10%', cellsalign: 'center', align: 'center' },// 5

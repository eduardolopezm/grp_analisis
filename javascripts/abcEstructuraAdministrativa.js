...
$(document).ready(function() {
    ...
    $('#nuevo').on('click',function(){
        $('#tituloModal').html('Nueva Estructura Administrativa');
        $('#modalAdministrativa').modal('show');
    });
    // comportamiento del boton de guardado
    $('#guardar').on('click',function(){
        var params = getParams(idForma), campos=getMatchets(idForma), msg=''
            ,nombreCampo={'ur':'UR','aux':'Auxiliar 1','ef':'EF'}
    ...
    $(document).on('cellselect','#tablaGrid',function(e){
        ...
        .then(function(res){
            ...
            if(res.success){
                ...
                $('#modalAdministrativa #tituloModal').html("Edición Estructura Administrativa");
                $('#modalAdministrativa').modal('show');
            }
        });
    })
...
function inicioPanel() {
    ...
    this.rootFu = 0;
	this.url = getUrl();
	this.modelo = this.url+'/modelo/abcEstructuraAdministrativaModelo.php';
    ...
    $('#modalAdministrativa').on('hidden.bs.modal',function(){
        var limpiarSelects = ['aux','ef'];
        $.each(limpiarSelects,function(index,el){ $('#'+el).multiselect('select',0).multiselect('rebuild'); });
        // $('#aux').multiselect('select','-1').multiselect('rebuild');
        $('#modalAdministrativa').find('#identificador').remove();
    });
...
function llenaTabla(data) {
	...
	var data = data||[], el = 'contenedorTabla', tabla = 'tablaGrid', nameExcel = 'Estructura Administrativa'
        , tblObj, tblTitulo, tblExcel=[0,1,2], tblVisual=[0,1,2,3]; //,4
    tblObj = [
        { name: 'ur', type: 'string'},// 0
        { name: 'aux', type: 'string'},// 1
        { name: 'ef', type: 'string'},// 2
    ...
    tblTitulo = [
        { text: 'UR', datafield: 'ur', editable: false, width: '31%', cellsalign: 'center', align: 'center' },// 0
        { text: 'Auxiliar 1', datafield: 'aux', editable: false, width: '31%', cellsalign: 'center', align: 'center' },// 1
        { text: 'EF', datafield: 'ef', editable: false, width: '31%', cellsalign: 'center', align: 'center' },// 2
        //{ text: 'Modificar', datafield: 'modificar', editable: false, width: '20%', cellsalign: 'center', align: 'center' },// 3
        { text: 'Eliminar', datafield: 'eliminar', editable: false, width: '7%', cellsalign: 'center', align: 'center' },// 4 antes todas a width: '20%'
    ];

...
function fnMostrarDatos(cg){
	...
	$.ajax({
	      method: "POST",
	      dataType:"json",
	      url: "modelo/abc_aux2_modelo.php",
	      data:dataObj
	  })
	.done(function( data ) {
		//console.log("Bien");
...
function fnAgregarCatalogoModal(){
	...
	var titulo = '<h3><span class="glyphicon glyphicon-info-sign"></span> Agregar Auxiliar 2 </h3>';
...
function fnAgregar(){
 ...
 //cgnuevo=('0' + cgnuevo).slice(-2);//agregar  cero
	...
	/*if ( (cgnuevo == "") && (descripcion == "")) {
		...
		$('#ModalCg').modal('hide');
		muestraMensaje('Debe agregar Auxiliar 2 y Descripción para realizar el proceso', 3, 'msjValidacion', 5000);
		return false;
		
	}else if (cgnuevo == "") {
		$('#ModalCg').modal('hide');
		muestraMensaje('Debe agregar Auxiliar 2 para realizar el proceso', 3, 'msjValidacion', 5000);
		return false;
	}else if (descripcion == "") {
		$('#ModalCg').modal('hide');
	...
	if (cgnuevo == "" || descripcion == "") {
		if (cgnuevo == "" ) {
			msg += '<p>Falta capturar la clave Auxiliar 2 </p>';
		}
		if (descripcion=="" ) {
...
function fnModificar(cg){
	...
	.done(function( data ) {
	    ...
	    if(data.result){
			...
			var titulo = '<h3><span class="glyphicon glyphicon-info-sign"></span> Modificar Auxiliar 2</h3>';
			$('#ModalCg_Titulo').empty();
		    $('#ModalCg_Titulo').append(titulo);
			$('#ModalCg').modal('show');
...
function fnEliminar(cg){
	...
	var mensaje = 'Desea eliminar el Auxiliar 2 <b> '+cg+'</b>';

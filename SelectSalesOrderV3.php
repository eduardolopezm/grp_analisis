...
include ('includes/SendInvoicingv2.inc');
...
if (isset($orden) and isset($ventaperdida) and $ventaperdida=='yes'){
	...
	prnMsg(_('El Numero de Pedido') . ' ' . $orden . ' ' . _('se ha convertido a facturado y la cantidad restante de los productos se considerara como venta perdida'),'success');
     ...
     echo '<meta http-equiv="Refresh" content="2; url=' . $rootpath .
			'/SelectSalesOrderV3.php?OrderNumber=' . $orden . '&UnidNeg=' . $tagrefen 
			. '&FromDia=' . $FromDia 
			. '&FromMes=' . $FromMes 
			. '&FromYear=' . $FromYear 
			. '&ToDia=' . $ToDia
...
// 		
		...
		if($tipofacturacionxtag==2){
			//Array con xml y cadena original
			echo $OrderNox;
			...
			$array=generaXMLIntermedio($XMLElectronico,$arrayGeneracion["cadenaOriginal"],$arrayGeneracion["cantidadLetra"],$OrderNox,$db);
// 			generacionReporte($XMLImpresion,null,"pdf");
...
// 			echo  '<pre><br/>'.htmlentities($XMLImpresion);
// 			exit;
			
			$handle = fopen("/var/www/html/xmlsdi/$serie$folio.xml", 'w');
			fwrite($handle, $XMLElectronico);
			fclose($handle);
...
/*esto lo aumente*/ $tienepagares2 = "&nbsp;";
		...
		if ($Quotations!=4 and $Quotations!=5 and $Quotations!=6){
			if (Havepermission($_SESSION['UserID'],196, $db)==1){
				...
				if ($Quotations==0 && Havepermission($_SESSION['UserID'],677, $db)==1){
					$Cancelar = $rootpath . '/SelectSalesOrderV3.php?orderno='. $myrow['orderno'];
		...
		if (Havepermission($_SESSION['UserID'],203, $db)==1 and $Quotations==6 ){
			...
			$VentaPerdida= $rootpath . '/SelectSalesOrderV3.php?ventaperdida=yes&orderno='.
			$myrow['orderno'].
			'&tagrefen='.$tagref.
			'&iddocto='.$idfactura.
			'&serie='.$serie.
		...
		if ($tipofacturacionxtag==2 or $tipofacturacionxtag==1 or $tipofacturacionxtag==4 ) {
			$ReenvioXSA= $rootpath . '/SelectSalesOrderV3.php?orderno='.
			$myrow['orderno'].
			'&tagrefen='.$tagref.
			'&iddocto='.$idfactura.
			'&serie='.$serie.
